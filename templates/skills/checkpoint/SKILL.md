...
1. **Resolve the caller repository and target path.** From the caller's original working directory, run `git rev-parse --show-toplevel`. If it succeeds with a non-empty path, use that path as `<repo-root>` and write to `<repo-root>/docs/checkpoints/`. Use Git's repository discovery rather than inspecting `.git` or walking parent directories yourself: this handles both a normal `.git` directory and a linked worktree's `.git` file without accidentally selecting a parent repository. **If the command fails or returns an empty path (caller is outside any Git repository), refuse and stop** — report that `/checkpoint` needs a Git repository and do not write anywhere else. Checkpoints are project-scoped; never fall back to `~/.claude/checkpoints/` or any other path shared across projects/agents. Filename: `<name>.md` if a name was provided, else `YYYY-MM-DD-HHMM.md`.
