...
**Opt-in skill classes get their own sentinel, never `AllSkills`.** `agent`/`agent-new` (the persistent sub-agent tree — `AllAgentTreeSkills` in `lib/manifest.js`, `lib/agent-tree.js`) are multi-file skill trees under `templates/skills/agent(-new)/` installed into the same `~/.claude/skills/` directory as regular skills, but they are deliberately **not** in `AllSkills` and are stamped with a distinct sentinel (`<!-- cah-agent-tree:v1 -->`, not `<!-- cah-skill:v1 -->`). This is load-bearing, not incidental: `writeSkills`'/`removeSkills`' orphan-pruning is scoped to `AllSkills` + `SetForSkill`, so a directory carrying a different sentinel is classified `foreign` and never touched — a plain `--only skills` install/reinstall can never prune the agent-tree directories just because they're absent from the main registry. Never merge this class into `AllSkills` — it would default-install (defeating the whole point of the opt-in), and it would also make it visible to the main orphan sweep, which is exactly the collision the separate sentinel exists to prevent. Follow this same shape (own registry array + own sentinel + own writer/remover module) for any future skill or class that must be genuinely opt-in yet still live in `~/.claude/skills/`.
...
| `lib/agent-tree.js` | `writeAgentTree` / `removeAgentTree` for the opt-in persistent sub-agent tree skills (`agent`, `agent-new`, `AllAgentTreeSkills`) — same shape as `lib/skills.js` but with its own sentinel so it's invisible to `writeSkills`'/`removeSkills`' `AllSkills`-scoped orphan-pruning |
...
- **README install examples must cover every installable artefact.** The `Use` section must contain a one-line `npx cah install --only <name>` example for **every** skill in `AllSkills`, for `bins`, and for the opt-in classes (`commands`, `codex-agents`, `agent-tree`). Class `agents` is exempt — it has no point-install story (it's in the default set). When adding a new skill to `AllSkills`, add its example line in `README.md` in the same PR; CI does not enforce this, the project does.
- **The model-commands table, the Codex-agents table, and every "N commands/agents" count in README.md are generated, not hand-written.** They live between `<!--gen:table:KEY-->`/`<!--/gen:table:KEY-->` and `<!--gen:count:KEY-->N<!--/gen-->` markers. After editing `AllModelCommands` or `AllCodexAgents`, run `npm run gen:docs` — never hand-edit the content between those markers, it will just be overwritten. `test/gen-docs.test.js` runs `npm run gen:docs:check` and fails the suite if README.md has drifted from the manifest.
- **Opus and Fable use "releases behind top" command/agent naming, not a version number.** `o1*` means "whichever Opus was top before the current one", `o2*` the one before that, etc. — `lib/manifest.js`'s Opus block must be renumbered (not the model ids) every time a new Opus generation ships and everyone shifts back one slot. Fable follows the identical convention (`f1*` = previous top Fable), renumbered the same way when a new Fable generation ships. Other Claude families (Sonnet, Haiku) still encode the literal version number (`s45*`, `h45*`).
...
- **Opt-in classes (`codex-agents`, `agent-tree`, `commands`) share one flag-handling path.** `lib/cli.js`'s `OPT_IN_FLAG_CLASSES` + `applyOptInFlags()` is the single place that implements "`--<flag>` alone replaces the default class set; `--<flag>` combined with `--only` adds to it." Add a new opt-in class here rather than hand-rolling another `applyXFlag` function. Opt-in classes are listed in `VALID_CLASSES` but never in `SELECTOR_CLASSES` (the default-install set) — that's what keeps a bare `cah install` from touching them. `enumerate()`/`cmdDoctor()` also gate their rows: an opt-in class with zero installed items is excluded from the health count entirely (absence isn't a health problem for something that's opt-in), matching the existing `hasCodexAgents`/`hasAgentTree`/`hasModelCommands` pattern. `commands` joined the opt-in set because Claude Code's interactive TUI path silently ignores the per-command `model:`/`effort:` frontmatter override these files rely on ([anthropics/claude-code#81318](https://github.com/anthropics/claude-code/issues/81318)) — the `agents` half of the same registry is unaffected (the `Agent`-tool dispatch path honors the override) and stays in the default set.
- **`docs/plan-agent-tree.md` and `docs/test-plan-agent-tree.md` are contributor-facing design docs for the agent-tree feature, not runtime artifacts.** They document the architecture, invariants, and the reasoning behind the directory-boundary and permission-model choices in `lib/agent-tree.js`'s `assets/agent-tree.js` engine — read them before changing that engine's behavior. `cah` never installs them anywhere; they just live in this repo's `docs/`.
