...
- **Opus uses "releases behind top" command/agent naming, not a version number.** `o1*` means "whichever Opus was top before the current one", `o2*` the one before that, etc. — `lib/manifest.js`'s Opus block must be renumbered (not the model ids) every time a new Opus generation ships and everyone shifts back one slot. Other Claude families (Sonnet, Haiku) still encode the literal version number (`s45*`, `h45*`).
