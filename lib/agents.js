...
import {
  ...
  removeOwnedRegularFile, waitForTestInterlock,
...
function buildAgentBody(mc) {
  ...
  const description = mc.effort == null ? `${mc.model} (${mc.display})` : `${mc.model} effort=${mc.effort} (${mc.display})`;
  const invocation = mc.effort == null
    ? 'You are a delegated worker running on the selected model.\n\n'
    : `You are a delegated worker invoked with reasoning effort: ${mc.effort}.\n\n`;
  return (
    '---\n' +
    `name: ${mc.name}\n` +
    `description: ${description} — delegate task in isolated context\n` +
    `model: ${mc.model}\n` +
    effort +
    '---\n\n' +
    invocation +
...
export function writeModelAgents(_templates, scope) {
  ...
  // resolveAgentsDir() applies the strict-scope guard; writeFileAtomic
  ...
  for (const mc of AllModelCommands) {
    ...
    const ownership = classifyContent(snapshot.present, snapshot.content, SetForModelAgent);
    ...
    writeFileAtomic(path, buildAgentBody(mc), {
  ...
  const recovery = [];
  mergeOrphanReport(skipped, recovery, dir, orphanReport);
  return { written, skipped, recovery, pruned: orphanReport.pruned };
}
...
export function removeModelAgents(scope) {
  ...
  mergeOrphanReport(skipped, recovery, dir, orphanReport);
  return { removed, skipped, recovery, pruned: orphanReport.pruned };
}

function mergeOrphanReport(skipped, recovery, dir, report) {
