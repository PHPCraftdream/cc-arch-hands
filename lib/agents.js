...
export function writeModelAgents(_templates, scope, options = {}) {
  ...
  const orphanReport = pruneOrphans(
    ...
    SetForModelAgent, { testInterlock: options.testInterlock },
  );
  const recovery = [];
...
export function removeModelAgents(scope, options = {}) {
  const dir = scope.resolveAgentsDir();
  let removed = 0;
  const skipped = [];
  const recovery = [];
