...
- Run the \`cli-run\` launcher in a persistent Codex Background Terminal with PTY enabled, even when the user only says "run". Keep that terminal visible in \`/ps\` until the worker exits. The PTY is for the launcher terminal only: job processes receive ignored stdin, and their combined stdout/stderr are written to per-job logs; do not expect interactive stdin or a PTY for individual jobs.
- In that same terminal, launch \`cli-run\` with the complete command job array on stdin. Parse its launch JSON, then wait on that exact worker PID in the same shell (PowerShell: \`Wait-Process -Id ([int]$started.pid)\`). Preserve the terminal/session handle; this blocking OS wait is not polling.
- Never use a detached-only or ordinary foreground fallback. If no native Background Terminal or PTY with a live session handle is available, stop and ask.
