...
function preflightBinLeaves(binDir) {
  ...
  for (const file of BinFiles) {
    ...
    const structural = structuralLeafRejection(path);
    if (structural !== null) throwForeignLeaf(file, path, structural);
...
// Read-only structural classification of a declared runtime leaf, shared by
// install preflight and the doctor/list health rows: a target install would
// reject — anything that is not a plain regular file with exactly one hard
// link — must never be reported as a healthy owned file. Returns a
// human-readable reason string, or null when the leaf is absent (absence is
// not a rejection) or structurally acceptable.
export function structuralLeafRejection(path) {
  const leafStat = lstatNoFollow(path);
  if (leafStat === null) return null;
  if (leafStat.isSymbolicLink()) return 'symbolic link';
  if (leafStat.isDirectory()) return 'directory';
  if (!leafStat.isFile()) return 'non-regular entry';
  if (leafStat.nlink > 1n) return 'multi-hardlink regular file';
  return null;
}

function throwForeignLeaf(file, path, structural) {
  if (file === BoundaryFile) {
    throw new Error(
      `foreign package boundary at ${path}: ${structural}; managed companion bins require the cah-owned ESM package.json`,
    );
  }
  throw new Error(
    `foreign managed runtime leaf at ${path}: ${structural}; refusing a mixed companion-bin closure`,
  );
}
...
// Lease quarantines land beside the lock that produced them, so the fixed
// cah-owned namespaces below `cache/` are inspected alongside it: the stamp
// locks and sidecars are the highest-volume producers, the per-render status
// sidecars (`rate-context`) and the marker claim namespaces follow. Producers
// outside the managed tree (`~/.claude/cah-bin.lock`,
// `~/.claude/settings.json.probe-lock`) are deliberately out of scope: they
// are low-volume and scanning `~/.claude` itself would put user-owned space on
// the maintenance path for two rarely-quarantined locks.
const CACHE_SCAN_SUBDIRS = ['stamp-state', 'update-markers', 'hint-markers', 'rate-context'];
...
function mergeCacheMaintenance(
  ...
  for (const root of [cacheDir, ...CACHE_SCAN_SUBDIRS.map((name) => join(cacheDir, name))]) {
    if (root !== cacheDir) {
      // A sub-namespace is optional runtime state: absent or non-directory
      // entries are skipped silently, exactly like the reserved cache itself.
      try {
        const stat = lstatNoFollow(root);
        if (stat === null || !stat.isDirectory()) continue;
      } catch (error) {
        markMaintenanceIncomplete(maintenance, root, error);
        continue;
      }
    }
    let report;
    try {
      report = maintainRecoveryArtifacts(root, {
    ...
    // Merge each scanned root once into the structured maintenance contract.
    ...
    mergeMaintenanceReport(maintenance, report,
      (path) => toBinRelative(binDir, path || root));
