...
function companionImportGraph(files, sourceRoot = PACKAGE_ROOT) {
  ...
  for (const file of files) {
    ...
    // Node's ESM interpretation is part of every copied JavaScript leaf's
    // runtime contract. Treat the managed boundary as a real dependency so
    // rollback cannot leave a surviving leaf without its module boundary.
...
function writeBinsUnlocked(binDir, sourceRoot, lease, options = {}) {
  ...
  try {
    ...
    const rollback = rollbackPublishedLeaves(
      published, snapshots, sourceRoot, lease, binDir, dependencyGraph, options,
    );
...
function rollbackPublishedLeaves(
  ...
  for (const dest of dependencyFailures) {
    republishConnectedGeneration(
      dest, published, snapshots, sourceRoot, lease, binDir, dependencyGraph, report, options,
    );
  }
...
function noteDependencyRollbackFailure(dest, failures) {
  if (dest === 'package.json' || dest.startsWith('lib/')) failures.add(dest);
}

function republishConnectedGeneration(
  failedDest, published, snapshots, sourceRoot, lease, binDir, dependencyGraph, report, options,
) {
  ...
  const items = new Map(publishedByDest);

  // A leaf that had not yet been reached by forward publication can still be
  // an already-installed member of the component. Include it when its prior
  // generation existed; otherwise a failed dependency would strand that old
  // importer beside the new dependency.
  for (const file of BinFiles) {
    if (!component.has(file.dest) || items.has(file.dest)) continue;
    const prior = snapshots.get(file.dest);
    if (prior?.present) {
      items.set(file.dest, {
        file,
        path: join(binDir, file.dest),
        prior,
        publication: null,
      });
    }
  ...
  for (const file of BinFiles) {
    if (!component.has(file.dest)) continue;
    ...
    if (item.publication && samePublicationState(current, item.publication)) continue;
    if (current.present && !sameRollbackState(current, item.prior)) {
      recordRepublishFailure(report, item, binDir, null, 'surviving-successor');
      recordRollbackSurvivor(report, item, binDir, 'surviving-successor');
      continue;
    }
    ...
    let payload;
    try {
      payload = item.publication?.content
        || injectSentinel(readFileSync(join(sourceRoot, item.file.src)));
    } catch (error) {
      recordRepublishFailure(report, item, binDir, error, 'source-unavailable');
      continue;
    }
    ...
    const reachedGeneration = after.present
      && after.contentDigest === (item.publication?.contentDigest
        || contentDigest(payload));
    if (actionError || !reachedGeneration) {
      ...
      if (after.present && !sameRollbackState(after, item.prior)) {
        recordRollbackSurvivor(report, item, binDir, 'republish-failed');
      }
      continue;
    }
...
function samePublicationState(current, publication) {
