...
function writeBinsUnlocked(binDir, sourceRoot, lease, options = {}) {
  ...
  try {
    ...
    const boundaryPublication = publishBinFile(
      ...
      () => assertBinLifecycleOwnership(lease, binDir),
    );
    published.push({
      file: BoundaryFile,
      path: join(binDir, BoundaryFile.dest),
    ...
    for (const f of BinFiles) {
      ...
      options.testInterlock?.('binstall-before-leaf-write');
      requireBinLifecycleLease(lease, binDir);
    ...
    mergeCacheMaintenance(
      binDir, recovery, maintenance, () => assertBinLifecycleOwnership(lease, binDir),
    );
    ...
    requireBinLifecycleLease(lease, binDir);
      rollbackPublishedLeaves(published, lease, binDir, options);
    throw error;
  }
}
...
function publishBinFile(binDir, sourceRoot, file, snapshot, assertOwnershipCallback = null) {
...
function rollbackPublishedLeaves(published, lease, binDir, options = {}) {
...
function mergeCacheMaintenance(binDir, recovery, maintenance, assertOwnershipCallback = null) {
  ...
  let report;
  try {
    report = maintainRecoveryArtifacts(cacheDir, { assertOwnership: assertOwnershipCallback });
  } catch (error) {
    if (error?.leaseLost) throw error;
