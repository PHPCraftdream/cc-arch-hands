...
function republishConnectedGeneration(
  failedDest, published, snapshots, generation, lease, binDir, dependencyGraph, report, options,
) {
  ...
  for (const file of BinFiles) {
    ...
    items.set(file.dest, publishedByDest.get(file.dest) || {
      ...
      prior: snapshots.get(file.dest),
      publication: null,
    });
  }

  // Repair is a generation-wide transaction. Establish every member's state
  // before the first repair write. In particular, an unknown successor at the
  // synthetic package boundary or any shared dependency invalidates the whole
  // target generation; publishing a later importer would otherwise create a
  // runtime that mixes incompatible generations.
  ...
  for (const file of BinFiles) {
    ...
    if (item.publication && samePublicationState(snapshot, item.publication)) {
      states.set(file.dest, 'target');
    } else if (sameRollbackState(snapshot, item.prior)) {
      states.set(file.dest, 'ready');
    } else {
      unproved.push({ item, reason: 'unproved-target' });
    }
  }
  if (unproved.length > 0) {
    for (const entry of unproved) {
      ...
      if (!report.failed.some((failure) => failure.dest === detail.dest
  ...
  for (const file of BinFiles) {
    if (!component.has(file.dest)) continue;
    const item = items.get(file.dest);
    if (states.get(file.dest) === 'target') continue;

    const dependency = [...(dependencyGraph.get(file.dest) || [])]
      .find((dest) => states.get(dest) !== 'target');
    if (dependency) {
      states.set(file.dest, 'blocked');
      recordRepublishFailure(report, item, binDir, null, 'dependency-unavailable');
      continue;
    }
    ...
    const reachedGeneration = after.present
      && after.contentDigest === contentDigest(payload)
      && after.contentBytes === payload.byteLength;
