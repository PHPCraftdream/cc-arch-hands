...
function withBinLifecycleLease(binDir, operation) {
  const lease = acquireLease(binLifecycleLockPath(binDir), {
    ...
    // live owner is reclaimed only once its absolute lease has expired.
...
export function writeBins(binDir, sourceRoot) {
  ...
  return withBinLifecycleLease(binDir, () => writeBinsUnlocked(binDir, sourceRoot));
}

function writeBinsUnlocked(binDir, sourceRoot) {
...
export function removeBins(binDir) {
  ...
  return withBinLifecycleLease(binDir, () => removeBinsUnlocked(binDir));
}

function removeBinsUnlocked(binDir) {
...
function mergeCacheMaintenance(binDir, recovery, maintenance) {
  ...
  mergeMaintenanceReport(maintenance, report, (path) => toBinRelative(binDir, path));
  maintenance.incomplete ||= Boolean(report.incomplete);
  maintenance.truncated ||= Boolean(report.truncated);
  maintenance.visits += report.visits || 0;
  for (const failure of report.failures || []) {
    if (maintenance.failures.length < 8) {
      maintenance.failures.push({
        path: toBinRelative(binDir, failure.path || cacheDir),
        code: failure.code || 'UNKNOWN',
      });
    }
  }
  for (const field of ['swept', 'preserved', 'recovery']) {
    for (const path of report[field] || []) {
      addUnique(maintenance[field], toBinRelative(binDir, path));
    }
  }
  for (const path of report.unprovedTemps || []) {
    const relativePath = toBinRelative(binDir, path);
    addUnique(maintenance.unprovedTemps, relativePath);
