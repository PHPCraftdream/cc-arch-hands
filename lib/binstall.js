...
export function writeBins(binDir, sourceRoot) {
  ...
  try {
    ...
    // Test-only pause used to replace the boundary immediately after the
    // first publication. The subsequent exact check must fail before leaves
    // can be reported as a successful install.
...
function assertPublishedBoundary(path, publication) {
  ...
  if (!current.present
      ...
      || !current.content.equals(publication.content)) {
...
function rollbackPublishedLeaves(published) {
  for (const item of [...published].reverse()) {
    ...
    if (!current.present || !sameFileIdentity(current.expectedDestination.identity, item.publication.identity)) {
    ...
    try {
      if (item.prior.present) {
        ...
        removeOwnedRegularFile(item.path, item.publication.identity);
      }
    } catch {
...
export function removeBins(binDir) {
  ...
  for (const sub of Object.keys(OwnedDirs)) {
    ...
    const known = sub ? new Set() : new Set(['bin', 'lib']);
    const orphanReport = pruneOrphans(dir, known, SetForBin);
