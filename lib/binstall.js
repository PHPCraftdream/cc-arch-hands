  ...
  for (const dest of dependencyFailures) {
    republishConnectedGeneration(
      dest, published, snapshots, generation, lease, binDir, dependencyGraph, report, options, {
        ...
        assertOwnership: assertBinLifecycleOwnership,
...
export function removeBins(binDir, _sourceRoot = undefined, maybeOptions = undefined) {
  ...
  const sourceRoot = typeof _sourceRoot === 'string' ? _sourceRoot : PACKAGE_ROOT;

  return withBinLifecycleLease(
    binDir, (lease) => removeBinsUnlocked(binDir, lease, options, sourceRoot), options,
  );
}

function removeBinsUnlocked(binDir, lease, options = {}, sourceRoot = PACKAGE_ROOT) {
  ...
  const snapshots = preflightBinLeaves(binDir);
  const dependencyGraph = getBinFileImportGraph(BinFiles, sourceRoot);
  const protectedBy = new Map();
  let removed = 0;
  const skipped = [];
  const recovery = [];
  ...
  for (const file of [...BinFiles].reverse()) {
    ...
    if (result === true) {
      ...
      // A surviving successor may be an importer; protect its closure first.
      if (stablePathExists(path)) protectDependencies(file.dest, dependencyGraph, protectedBy);
    }
  }
