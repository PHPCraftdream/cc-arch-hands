...
import { acquireLease, leaseOwned, releaseLease } from './lease-lock.js';
...
export function acquireSettingsLock(settingsPath, options = {}) {
  ...
  for (const key of [
    'staleAfterMs', 'fenceSuffix', 'interlockPhase', 'releaseInterlockPhase',
  ...
  for (;;) {
    ...
    const lease = acquireLease(lockPath, {
      staleAfterMs, fenceSuffix: '.stale.', kind: SETTINGS_LOCK_KIND, ...forward,
    });
    if (lease) {
      return {
        path: lease.path,
