...
export function writeSkills(templates, scope, options = {}) {
  ...
  for (const name of names) {
    ...
    for (const rel of listDirectQuarantineEntries(destDir)) {
...
function captureDestinationSnapshot(path) {
  ...
  return { exists: info !== null, identity: info && info.isFile() ? info : null };
}

function sameDestinationSnapshot(left, right) {
  if (left.exists !== right.exists) return false;
  return !left.exists || sameFileIdentity(left.identity, right.identity);
}
...
function listDirectQuarantineEntries(dir) {
  let entries;
  try {
  ...
  return entries.filter((entry) => isQuarantineName(entry.name)).map((entry) => {
    if (!entry.isDirectory()) return entry.name;
    try {
      return readdirSync(join(dir, entry.name), { withFileTypes: true })
        .some((child) => child.name === 'payload')
        ? `${entry.name}/payload`
        : entry.name;
    } catch (e) {
      if (e.code === 'ENOENT') return entry.name;
      throw e;
    }
  });
}
...
export function removeSkills(templates, scope, options = {}) {
  ...
  for (const name of managed) {
    ...
    if (extras.length > 0) {
      ...
      removeCapturedFiles(destDir, snapshots, root, expectedManifest, preserved);
      addUnique(preserved, name);
    } else {
      waitForTestInterlock('remove-before-owned-delete');
      const removedFiles = removeCapturedFiles(
        destDir, snapshots, root, expectedManifest, preserved,
      );
      if (!removedFiles) {
        addUnique(preserved, name);
        continue;
...
function removeCapturedFiles(destDir, snapshots, root, expectedManifest, preserved) {
  ...
  for (const [rel, identity] of entries) {
    ...
    const result = removeOwnedRegularFile(path, identity, { parentIdentities });
    if (result !== true) {
      if (result?.preservedPath) {
        const rel = relative(root, result.preservedPath).split(sep).join('/');
        addUnique(preserved, rel);
      }
      return false;
    }
  }
