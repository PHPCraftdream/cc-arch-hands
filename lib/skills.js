...
import {
  readFileMaybe, pruneOrphanDirs, writeFileAtomic,
  sameFileIdentity,
  removeOwnedRegularFile, removeEmptyDirectory,
  captureDirectoryIdentities, directoryIdentitiesMatch,
  captureRegularFileSnapshot, isQuarantineName, isQuarantinePath,
...
export function writeSkills(templates, scope, options = {}) {
  ...
  const catalog = options.catalog ?? AllSkills;
  const templateKind = options.templateKind ?? 'skills';
  const root = canonicalizeManagedRoot(options.root ?? scope.resolveSkillsDir());

  const subset = options.subset
    ? new Set([...options.subset].filter((s) => catalog.includes(s)))
    : null;
  const names = subset ? [...subset] : catalog;
  ...
  const trees = new Map(names.map((name) => {
    ...
    return [name, validateSkillTree(templates, name, destDir, templateKind)];
  }));
  ...
  for (const name of names) {
    ...
    mergeRecoveryMaintenance(recovery, preserved, root, destDir, options.recovery, maintenance);
    const manifestPath = join(destDir, SKILL_MANIFEST_LEAF);
    assertSafePath(manifestPath, root, 'file');
    const manifestBefore = captureDestinationSnapshot(manifestPath);
    const [present, content] = readFileMaybe(manifestPath);
    const manifestAfter = captureDestinationSnapshot(manifestPath);
    if (!sameDestinationSnapshot(manifestBefore, manifestAfter)) {
    ...
    manifestSnapshots.set(name, manifestAfter);
  }
  ...
  for (const name of names) {
    ...
    for (const file of trees.get(name)) {
      ...
      snapshots.set(
        file.relPath,
        file.relPath === SKILL_MANIFEST_LEAF
          ? manifestSnapshots.get(name)
          : captureDestinationSnapshot(destination),
      );
    }
    destinationSnapshots.set(name, snapshots);
  }

  for (const name of names) {
    if (initialOwnership.get(name) === Ownership.foreign) {
      addUnique(skipped, name);
      continue;
    }
    const destDir = resolve(root, name);
    assertSafePath(destDir, root, 'directory');
    ...
    for (const f of files) {
      ...
      options.testInterlock?.('write-before-owned-write');
      ...
      writeFileAtomic(destination, payload, {
        createParents: false,
        parentIdentities,
        expectedDestination: destinationSnapshots.get(name).get(f.relPath),
  ...
  const orphanReport = pruneOrphanDirs(
    root, new Set(catalog), SKILL_MANIFEST_LEAF, SetForSkill,
    { recovery: options.recovery, testInterlock: options.testInterlock },
  );
  const { pruned, preserved: prunedKept, recovery: prunedRecovery } = orphanReport;
  mergeMaintenanceReport(maintenance, orphanReport.maintenance, (path) => path);
  for (const n of prunedKept) {
    // A deterministic file quarantine is already a complete recovery path;
    // do not decorate it as a directory summary or report a path that does
    // not exist. Directory names retain the human-readable summary.
    if (isQuarantinePath(n)) addUnique(preserved, n);
    else addUnique(preserved, `${n}/ (orphan dir with user files)`);
  }
  for (const path of prunedRecovery) addUnique(recovery, path);
  const rootMaintenance = maintainRecoveryArtifacts(root, options.recovery ?? {});
  mergeMaintenanceReport(maintenance, rootMaintenance,
    (path) => normalizedRelativePath(root, path));
  for (const path of rootMaintenance.recovery) {
    addUnique(recovery, normalizedRelativePath(root, path));
  }
  for (const path of rootMaintenance.preserved) {
    addUnique(preserved, normalizedRelativePath(root, path));
  }
  mergeMaintenanceReport(maintenance, { recovery }, (path) => path);
  return { written, skipped, pruned, preserved, recovery, maintenance };
}
 ...
 */
export function validateSkillTree(templates, name, destDir = null, templateKind = 'skills') {
  const files = templates.skillTree(name, templateKind);
  ...
  if (typeof templates.root === 'string') {
    const sourceManifest = join(templates.root, templateKind, name, SKILL_MANIFEST_LEAF);
    let info;
    try {
      info = lstatSync(sourceManifest, { bigint: true });
    } catch {
...
function captureDestinationSnapshot(path) {
  const info = lstatMaybe(path);
  if (info === null) {
    return { exists: false, identity: null, contentDigest: null, contentBytes: 0 };
  }
  if (!info.isFile()) {
    return { exists: true, identity: null, contentDigest: null, contentBytes: null };
  }
  const snapshot = captureRegularFileSnapshot(path);
  return snapshot.expectedDestination;
}

function sameDestinationSnapshot(left, right) {
...
function assertSafeDestinationRoot(root) {
  ...
  // The configured destination chain has already been canonicalized. Do not
  // reject trusted symlinked ancestors such as a symlinked home or /tmp;
  // managed descendants are checked below this real boundary.
...
// Resolve the trusted configured scope chain once. If the managed root or a
// component below it is absent, realpath the nearest existing ancestor and
// append only the missing suffix. This permits symlinked `.claude`/`skills`
// trees while keeping all managed descendants on one canonical path.
function canonicalizeManagedRoot(path) {
  ...
  return join(realBase, relative(cursor, configured));
}
...
function ensureSafeDirectory(path, anchor) {
  ...
  const anchorReadyAtStart = anchor === null || lstatMaybe(anchor) !== null;
  // An absent managed anchor is allowed to be created under its trusted
  // configured parent. Do not walk from there to filesystem root, where
  // harmless symlinked ancestors such as /tmp or a home directory would be
  // mistaken for an unsafe managed traversal.
  const trustedBoundary = anchor === null ? null : cursor;
  assertSafePath(cursor, anchorReadyAtStart ? anchor : trustedBoundary, 'directory');
  let anchorReady = anchorReadyAtStart;
  for (const dir of missing.reverse()) {
    try {
      mkdirSync(dir);
    } catch (e) {
      if (e.code !== 'EEXIST') throw e;
      // A concurrent creator is acceptable only after lstat and identity
      // rechecks prove that the component is still our real directory.
      const raced = lstatMaybe(dir);
      if (!raced || !raced.isDirectory() || raced.isSymbolicLink()) {
        throw new Error(`destination directory component changed concurrently: ${dir}`);
      }
      const racedIdentity = captureDirectoryIdentities([dir]);
      if (!racedIdentity) {
        throw new Error(`destination directory component changed concurrently: ${dir}`);
      }
      assertSafePath(dir, anchorReady ? anchor : trustedBoundary, 'directory');
      if (!directoryIdentitiesMatch(racedIdentity)) {
        throw new Error(`destination directory component changed concurrently: ${dir}`);
      }
      if (anchor !== null && samePath(dir, anchor)) anchorReady = true;
      continue;
    }
    const madeIdentity = captureDirectoryIdentities([dir]);
    assertSafePath(dir, anchorReady ? anchor : trustedBoundary, 'directory');
    if (!madeIdentity || !directoryIdentitiesMatch(madeIdentity)) {
      throw new Error(`destination directory component changed concurrently: ${dir}`);
    }
    if (anchor !== null && samePath(dir, anchor)) anchorReady = true;
  }
...
export function removeSkills(templates, scope, options = {}) {
  ...
  for (const name of names) {
    ...
    const manifestPath = join(destDir, SKILL_MANIFEST_LEAF);
    assertSafePath(manifestPath, root, 'file');
    const manifestSnapshot = captureRegularFileSnapshot(manifestPath);
    const ownership = classifyContent(
      manifestSnapshot.present, manifestSnapshot.content, SetForSkill,
    );
    if (ownership === Ownership.foreign) addUnique(skipped, name);
    if (ownership === Ownership.mine || ownership === Ownership.legacy) {
      managed.push(name);
      // This exact stable identity gates every later capture and deletion for
      // the skill. A successor manifest must never be reclassified as ours.
      manifestSnapshots.set(name, manifestSnapshot.expectedDestination);
    }
  }
  ...
  for (const name of managed) {
    ...
    const manifestPath = join(destDir, SKILL_MANIFEST_LEAF);
    assertSafePath(manifestPath, root, 'file');
    const expectedManifest = manifestSnapshots.get(name);
    if (!destinationMatchesSnapshot(manifestPath, expectedManifest)) {
      addUnique(preserved, name);
      continue;
    }
    ...
    options.testInterlock?.('remove-before-owned-capture');
    if (!destinationMatchesSnapshot(manifestPath, expectedManifest)) {
      addUnique(preserved, name);
      continue;
    }
    const snapshots = captureOwnedFiles(destDir, ownedRel, root, expectedManifest);
    // Capturing a successor's leaves is harmless only if they are never used;
    // reject the whole capture if the ownership marker changed while it ran.
    if (!destinationMatchesSnapshot(manifestPath, expectedManifest)) {
      addUnique(preserved, name);
      continue;
    }
    if (extras.length > 0) {
      ...
      removeCapturedFiles(destDir, snapshots, root, expectedManifest, recovery,
        options.testInterlock);
      addUnique(preserved, name);
    } else {
      options.testInterlock?.('remove-before-owned-delete');
      const removedFiles = removeCapturedFiles(
        destDir, snapshots, root, expectedManifest, recovery, options.testInterlock,
      );
      if (!removedFiles) {
        addUnique(preserved, name);
        continue;
      }
      options.testInterlock?.('remove-before-rmdir');
      ...
      for (const rel of [...expectedDirs].sort((a, b) => b.length - a.length)) {
        ...
        if (parentIdentities !== null && directoryIdentitiesMatch(parentIdentities)) {
          removeEmptyDirectory(parent, { parentIdentities, testInterlock: options.testInterlock });
        }
      }
      ...
      if (skillIdentities !== null && directoryIdentitiesMatch(skillIdentities)
          && removeEmptyDirectory(destDir, {
            parentIdentities: skillIdentities, testInterlock: options.testInterlock,
          })) removed++;
      else addUnique(preserved, name);
    }
  }
  const rootMaintenance = maintainRecoveryArtifacts(root, options.recovery ?? {});
  mergeMaintenanceReport(maintenance, rootMaintenance,
  ...
  mergeMaintenanceReport(maintenance, { recovery }, (path) => path);
  return { removed, skipped, preserved, recovery, maintenance };
}
...
function captureOwnedFiles(destDir, ownedRel, root, expectedManifest) {
  const snapshots = new Map();
  for (const rel of ownedRel) {
    const destination = resolve(destDir, rel);
    assertContained(destDir, destination, `skill file ${rel}`);
    assertSafePath(destination, root, 'file');
    if (rel === SKILL_MANIFEST_LEAF) {
      snapshots.set(rel, expectedManifest);
      continue;
    }
    const snapshot = captureRegularFileSnapshot(destination);
    if (snapshot.present && snapshot.expectedDestination.identity) {
      snapshots.set(rel, snapshot.expectedDestination);
    }
  }
  return snapshots;
}
...
function removeCapturedFiles(
  destDir, snapshots, root, expectedManifest, recovery, testInterlock = null,
) {
  const manifestPath = resolve(destDir, SKILL_MANIFEST_LEAF);
  const entries = [...snapshots].sort(([left], [right]) => {
    if (left === SKILL_MANIFEST_LEAF) return 1;
    if (right === SKILL_MANIFEST_LEAF) return -1;
    return 0;
  });
  for (const [rel, expected] of entries) {
    // Keep the original marker as the ownership authority until all other
    // leaves have been removed. Once the marker itself is last, no successor
    // can be mistaken for an owned file by this operation.
    if (!destinationMatchesSnapshot(manifestPath, expectedManifest)) return false;
    ...
    const result = removeOwnedRegularFile(path, expected, { parentIdentities, testInterlock });
    if (result !== true) {
      if (result?.preservedPath) {
        const recoveredRel = relative(root, result.preservedPath).split(sep).join('/');
        addUnique(recovery, recoveredRel);
      }
      return false;
    }
  }
...
function destinationMatchesSnapshot(path, expected) {
  if (!expected) return false;
  const current = captureDestinationSnapshot(path);
  return sameDestinationSnapshot(current, expected);
}
...
function mergeRecoveryMaintenance(
  recovery, preserved, root, path, options = undefined, maintenance = null,
) {
  const report = maintainRecoveryArtifacts(path, options ?? {});
  if (maintenance) {
    mergeMaintenanceReport(maintenance, report,
      (value) => normalizedRelativePath(root, value));
  }
  for (const artifact of report.recovery) {
    addUnique(recovery, normalizedRelativePath(root, artifact));
  }
  for (const artifact of report.preserved) {
    addUnique(preserved, normalizedRelativePath(root, artifact));
  }
}
