...
export function writeSkills(templates, scope, options = {}) {
  ...
  const root = canonicalizeManagedRoot(scope.resolveSkillsDir());
...
export function removeSkills(templates, scope, options = {}) {
  ...
  for (const name of managed) {
    ...
    if (extras.length > 0) {
      ...
      removeCapturedFiles(destDir, snapshots, root, expectedManifest, preserved, recovery,
        options.testInterlock);
      addUnique(preserved, name);
    } else {
      options.testInterlock?.('remove-before-owned-delete');
      const removedFiles = removeCapturedFiles(
        destDir, snapshots, root, expectedManifest, preserved, recovery, options.testInterlock,
      );
      if (!removedFiles) {
        addUnique(preserved, name);
        continue;
...
function removeCapturedFiles(
  destDir, snapshots, root, expectedManifest, preserved, recovery, testInterlock = null,
) {
  ...
  for (const [rel, expected] of entries) {
    ...
    if (result !== true) {
      if (result?.preservedPath) {
        const rel = relative(root, result.preservedPath).split(sep).join('/');
        addUnique(recovery, rel);
      }
      return false;
    }
  }
