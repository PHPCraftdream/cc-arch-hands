...
export function writeSkills(templates, scope, options = {}) {
  ...
  for (const n of prunedKept) {
    ...
    if (n.endsWith('.cah-owned-remove')) preserved.push(n);
    else preserved.push(`${n}/ (orphan dir with user files)`);
  }
  ...
  for (const [rel, identity] of entries) {
    ...
    if (result !== true) {
      if (result?.quarantinePath) {
        const rel = relative(root, result.quarantinePath).split(sep).join('/');
        if (!preserved.includes(rel)) preserved.push(rel);
      }
      return false;
    }
  }
