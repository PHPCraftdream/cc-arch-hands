...
export function removeSkills(templates, scope, options = {}) {
  ...
  for (const name of managed) {
    ...
    waitForTestInterlock('remove-before-owned-capture');
    if (!destinationMatchesSnapshot(manifestPath, expectedManifest)) {
      addUnique(preserved, name);
      continue;
    }
    ...
    if (extras.length > 0) {
      ...
      const removedFiles = removeCapturedFiles(
        destDir, snapshots, root, expectedManifest, preserved, recovery,
      );
      if (!removedFiles) {
        addUnique(preserved, name);
        continue;
...
function removeCapturedFiles(destDir, snapshots, root, expectedManifest, preserved, recovery) {
  ...
  for (const [rel, expected] of entries) {
    ...
    const result = removeOwnedRegularFile(path, expected, { parentIdentities });
    if (result !== true) {
