...
export function removeSkills(templates, scope, options = {}) {
  ...
  for (const name of managed) {
    ...
    if (extras.length > 0) {
      ...
      removeCapturedFiles(destDir, snapshots);
      preserved.push(name);
    } else {
      waitForTestInterlock('remove-before-owned-delete');
      removeCapturedFiles(destDir, snapshots);
      ...
      if (removeEmptyDirectory(destDir)) removed++;
...
function removeCapturedFiles(destDir, snapshots) {
  for (const [rel, identity] of snapshots) {
    removeOwnedRegularFile(resolve(destDir, rel), identity);
  }
}
