...
export function removeSkills(templates, scope, options = {}) {
  ...
  for (const name of managed) {
    ...
    if (extras.length > 0) {
      ...
      removeCapturedFiles(destDir, snapshots, root, expectedManifest);
      preserved.push(name);
    } else {
      waitForTestInterlock('remove-before-owned-delete');
      const removedFiles = removeCapturedFiles(destDir, snapshots, root, expectedManifest);
      if (!removedFiles) {
        preserved.push(name);
        continue;
      }
...
function removeCapturedFiles(destDir, snapshots, root, expectedManifest) {
  ...
  for (const [rel, identity] of entries) {
    ...
    if (!removeOwnedRegularFile(path, identity, { parentIdentities })) return false;
  }
  return true;
}
