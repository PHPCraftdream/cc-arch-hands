...
export function victimFencePath(sourcePath) {
  return join(dirname(sourcePath), `.cah-tmp-victim-${basename(sourcePath)}`);
}
...
function releaseEmptyVictimFence(fencePath) {
  try {
    rmdirSync(fencePath);
...
function fenceAndRemoveVictim(markerDir, state, context, sourcePath, helpers) {
  const fencePath = victimFencePath(sourcePath);
  ...
  if (!helpers.transactionStillCurrent(markerDir, state, context)) {
    releaseEmptyVictimFence(fencePath);
    return false;
  }
  ...
  if (finalFence.status !== helpers.absent || !fenceMatches(finalSource, state, helpers)) {
    ...
    if (finalFence.status === helpers.absent) releaseEmptyVictimFence(fencePath);
    return false;
  }
  context?.testInterlock?.('marker-capacity-victim-rename', 'before', 'marker-capacity');
  try { renameSync(sourcePath, victimFencePayload(fencePath)); } catch { return false; }
...
export function recoverVictimFence(markerDir, state, context, sourcePath, helpers) {
  ...
  if (fence.status === helpers.absent && fenceRoot.status === helpers.present) {
    return releaseEmptyVictimFence(fencePath);
  }
