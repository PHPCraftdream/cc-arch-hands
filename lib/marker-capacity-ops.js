...
export function inspectTransactionRetirement(transactionPath, entries,
  ...
  for (const name of entries.filter((value) => value.startsWith('.cah-retired-'))) {
    ...
    let rootEntries;
    try { rootEntries = readdirSync(rootPath); } catch { return { status: indeterminate }; }
  ...
  return found
    ? { status: present, ...found, reservations }
    : reservations.length
      ? { status: present, reservation: reservations[0], reservations }
      : { status: 'absent', reservations: [] };
}
  ...
  const releaseCreatedReservation = () => {
    ...
    try {
      ...
      if (current.status !== h.present || !current.identity?.isDirectory
          ...
          || readdirSync(retiredDir).length !== 0) return;
    ...
    if (retired.status !== h.present) {
      if (retiredRoot.status === h.present) {
        ...
        if (currentRoot.status !== h.present || !currentRoot.identity?.isDirectory
            ...
            || readdirSync(retiredDir).length !== 0
