import {
  linkSync, mkdirSync, renameSync, rmdirSync, unlinkSync, readdirSync,
...
export function inspectTransactionRetirement(transactionPath, entries,
  { inspectPath, parseTransactionState, present, indeterminate }) {
  let found = null;
  for (const name of entries.filter((value) => value.startsWith('.cah-retired-'))) {
    ...
    if (retired.status !== present) {
      if (found) return { status: indeterminate };
      found = { reservation: true, path: rootPath };
      continue;
    }
    const parsed = parseTransactionState(retired.content);
    if (!parsed || resolve(transactionRetirementPath(transactionPath,
      parsed.capacityLeaseGeneration)) !== resolve(rootPath) || found) {
      return { status: indeterminate };
    }
    found = { state: retired, path: rootPath, retiredPath: join(rootPath, 'transaction.json') };
  }
  return found ? { status: present, ...found } : { status: 'absent' };
}
...
export function retireTransactionState(markerDir, state, context, h) {
  ...
  const leaseValid = () => Boolean(state?.capacityLeaseGeneration
    && h.transactionLeaseValid(state, context) && h.transactionInspection(markerDir).status === h.present);
  ...
  try {
    ...
    if (retired.status === h.indeterminate) return false;
    if (retired.status !== h.present) {
      if (retiredRoot.status === h.present) {
        if (readdirSync(retiredDir).length !== 0 || !h.transactionStillCurrent(markerDir, state, context)) {
          return false;
        }
      ...
      if (canonical.status !== h.present || !parsed
          ...
          || !h.transactionStillCurrent(markerDir, state, context)) return false;
      context?.testInterlock?.('marker-capacity-transaction-retire', 'before', 'marker-capacity');
      if (!h.transactionStillCurrent(markerDir, state, context)) return false;
    ...
    if (emptyRoot.status !== h.present || !emptyRoot.identity?.isDirectory
        || readdirSync(retiredDir).length !== 0 || !h.transactionLeaseValid(state, context)) return false;
    rmdirSync(retiredDir);
    syncBestEffort(txDir);
    return true;
  } catch { return false; }
