...
export function makeCapacityRecoveryHelpers(h) {
  ...
  function transactionInspection(markerDir, {
    ...
    let retirement = inspectTransactionRetirement(txPath, entries, {
      ...
      present: h.present,
      indeterminate: h.indeterminate,
      validateState: (candidate) => stateBelongsToMarkerDir(candidate, markerDir),
    });
    ...
    if (state.status === h.present) {
      ...
      if (!parsed || !stateBelongsToMarkerDir(parsed, markerDir)) {
        return { status: h.indeterminate, state: null };
      }
    ...
    if (reconcileEmpty && retirement.records?.length && state.status === h.present
      ...
      if (!canonical || !stateBelongsToMarkerDir(canonical, markerDir)) {
        return { status: h.indeterminate, state: null };
      }
      ...
      if (expectedGeneration && assertOwnership()) {
        const reconciliation = reconcileExpiredRetirements(markerDir,
          ...
          expectedGeneration, assertOwnership, retirementLimit);
    ...
    return parsed && stateBelongsToMarkerDir(parsed, markerDir)
  ...
  function stateBelongsToMarkerDir(state, markerDir) {
    const root = resolve(markerDir);
    const inRoot = (path) => typeof path === 'string' && resolve(dirname(path)) === root;
    return h.validateStatePaths ? h.validateStatePaths(markerDir, state)
  ...
  function reconcileExpiredRetirements(markerDir, txPath, canonicalInspection, canonical,
    records, expectedGeneration, assertOwnership, limit = 16) {
    ...
    for (const record of ordered) {
      ...
      if (!stateBelongsToMarkerDir(record.state, markerDir)
      ...
      let finalEntries;
      try { finalEntries = readdirSync(record.path); } catch { return false; }
    ...
    return { ok: !blocked, complete: processed >= ordered.length || blocked };
  }
  ...
  function cleanupTransaction(markerDir, state, context) {
    ...
    try {
      if (!transactionStillCurrent(markerDir, state, context)) return false;
      const currentInspection = transactionInspection(markerDir);
      if (currentInspection.status !== h.present || !currentInspection.state
          ...
          || (h.inspectPath(h.capacityStatePath(markerDir)).status === h.present
            && currentInspection.retirement?.records?.length)) return false;
  ...
  function recoverCapacityTransaction(markerDir, state, capacityLease, nowMs, cfg, maintenance = null) {
    if (!stateBelongsToMarkerDir(state, markerDir)
