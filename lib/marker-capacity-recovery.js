  ...
  function reconcileExpiredRetirements(markerDir, txPath, canonicalInspection, canonical,
    ...
    for (const record of ordered) {
      ...
      try {
        ...
        // Sync the transaction directory that just lost this retirement
        // subdirectory, not txPath — this function's own txPath parameter is
        // the capacity-state *file* (h.capacityStatePath), a different value
        // than transactionInspection's same-named local (the transaction
        // directory). Opening a file with O_DIRECTORY is a hard ENOTDIR
        // wherever the platform enforces that flag.
        syncBestEffort(dirname(record.path));
      } catch { return { ok: false }; }
      processed += 1;
    }
    return { ok: !blocked };
