...
export function classifyPath(path, set, options = {}) {
  try {
    if (options.structural
      && structuralLeafRejection(path) !== null) {
      // Mirror install's structural runtime-leaf preflight: a leaf install
      // would reject is reported as foreign here too, and doctor/list stay
      // strictly read-only — this changes only the returned classification.
