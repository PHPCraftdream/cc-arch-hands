...
import { SetForModelCommand, SetForModelAgent, SetForSkill, classifyContent } from './sentinel.js';
...
Usage:
  cah install   [--global|--local] [--cwd PATH] [--templates DIR] [--only commands,agents,skills]
  cah uninstall [--global|--local] [--cwd PATH] [--only commands,agents,skills]
...
const VALID_CLASSES = ['commands', 'agents', 'skills'];
...
function reportClass(cls, verb, n, skipped) {
  process.stdout.write(`  ${cls}: ${verb} ${n}, skipped ${skipped.length} (foreign)\n`);
...
function cmdInstall(rest) {
  ...
  for (const cls of classes) {
    ...
    reportClass(cls, 'wrote', result.written, result.skipped);
  }
  return 0;
}
...
function enumerate(scope) {
  ...
  for (const mc of AllModelCommands) {
    rows.push({
      name: `a${mc.name}`,
      kind: 'agent',
      state: classifyPath(join(agentsDir, `a${mc.name}.md`), SetForModelAgent),
    });
  }
