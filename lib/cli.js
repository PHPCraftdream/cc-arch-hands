...
import { AllModelCommands, AllSkills, SkillDeps } from './manifest.js';
...
Usage:
  cah install   [--global|--local] [--cwd PATH] [--templates DIR] [--only SELECTOR]
  cah reinstall [--global|--local] [--cwd PATH] [--templates DIR] [--only SELECTOR]
  cah uninstall [--global|--local] [--cwd PATH] [--only SELECTOR]
 ...
 * (commands/agents/skills/bins) OR an individual skill name from AllSkills.
 *
...
export function parseOnly(only) {
  if (!only || only.trim() === '') {
    return { classes: [...VALID_CLASSES], skills: [] };
  }
...
function cmdInstall(rest) {
  ...
  let resolved;
  try {
    resolved = resolveDeps(parseOnly(vals.only));
  } catch (e) {
    process.stderr.write(`cah install: ${e.message}\n`);
    return 1;
  }
...
function cmdUninstall(rest) {
  ...
  try {
    parsed = parseScopeFlags(rest, {
      only: { type: 'string', default: '' },
      // Accepted but ignored: `cah reinstall --templates DIR` forwards its raw
      // argv to this phase, and uninstall has no use for the templates dir.
      // Without this, strict parseArgs would reject --templates and abort the
      // whole reinstall.
      templates: { type: 'string', default: '' },
    });
  } catch (e) {
