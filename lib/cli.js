...
import { AllModelCommands, AllCodexAgents, AllSkills, AllCodexSkills, SkillDeps } from './manifest.js';
...
import { writeCodexSkills, removeCodexSkills } from './codex-skills.js';
import {
  assertCodexInstructionsActive, inspectCodexInstructions, writeCodexInstructions,
  removeCodexInstructions, classifyCodexInstructions,
} from './codex-instructions.js';
import { validateSkillTree, writeSkills, removeSkills } from './skills.js';
import { writeBins, removeBins, BinFiles, structuralLeafRejection } from './binstall.js';
import {
  enableProbe,
  disableProbe,
  readProbeLog,
...
const GLOBAL_CODEX_SCOPE = new Scope({ global: true });

const USAGE = `cc-arch-hands - install Claude Code commands, agents and skills, plus optional Codex agents and skills.

Installed skills: ${AllSkills.join(', ')}.
Optional Codex skills: ${AllCodexSkills.join(', ')}.

Usage:
  cah install   [--global|--local] [--cwd PATH] [--templates DIR] [--only SELECTOR] [--commands] [--codex-agents] [--codex-skills]
  cah reinstall [--global|--local] [--cwd PATH] [--templates DIR] [--only SELECTOR] [--commands] [--codex-agents] [--codex-skills]
  cah uninstall [--global|--local] [--cwd PATH] [--only SELECTOR] [--commands] [--codex-agents] [--codex-skills]
...
const VALID_CLASSES = ['commands', 'agents', 'codex-agents', 'codex-skills', 'skills', 'bins'];
// Classes with their own --<flag>. Alone, a flag narrows the install to just
// that class; with --only it adds the class. Both Codex classes are opt-in (not in
// SELECTOR_CLASSES); `commands` is default again since anthropics/claude-code#81318
// was fixed (v2.1.280) — its flag stays for backward compatibility.
const OPT_IN_FLAG_CLASSES = ['codex-agents', 'codex-skills', 'commands'];
 ...
 * (commands/agents/skills/bins, plus the opt-in Codex classes) OR
...
// Applies any --<class> boolean flags.
// Used alone (no --only), a flag narrows the install to JUST that
...
function reportClass(
  ...
  if (maintenance && (maintenance.visits > 0
      ...
      || (maintenance.swept || []).length > 0
      || (maintenance.preserved || []).length > 0)) {
    process.stdout.write(
      ...
      + `swept ${(maintenance.swept || []).length}, `
      + `preserved ${(maintenance.preserved || []).length}\n`,
    );
  }
  // A destructive maintenance action inside a namespace the installer does
  // not own must be visible, never silent.
  for (const p of maintenance?.swept || []) {
    process.stdout.write(`    maintenance swept: ${p}\n`);
  }
  // The mirror obligation: an artifact the sweep examined but refused to
  // reclaim (a live owner's namespace, a fresh reservation, an unproved
  // publication fence) must be visible too, never silently preserved.
  // Artifacts the class lists above already name are not repeated here.
  const alreadyReported = new Set([
    ...skipped, ...(recovery || []), ...(preserved || []),
  ]);
  for (const p of maintenance?.preserved || []) {
    if (alreadyReported.has(p)) continue;
    process.stdout.write(`    maintenance preserved: ${p}\n`);
  }
...
function prepareInstall(rest) {
  const parsed = parseScopeFlags(rest, {
    ...
    'codex-skills': { type: 'boolean', default: false },
  ...
  if (resolved.classes.includes('codex-skills')) {
    scope.resolveCodexSkillsDir();
    assertCodexInstructionsActive(GLOBAL_CODEX_SCOPE);
    inspectCodexInstructions(GLOBAL_CODEX_SCOPE);
  }
  ...
  if (resolved.classes.includes('codex-skills')) {
    for (const name of AllCodexSkills) validateSkillTree(templates, name, null, 'codex-skills');
  }
    ...
    try {
      switch (cls) {
        ...
        case 'codex-agents':
          ...
          break;
        case 'codex-skills':
          result = writeCodexSkills(t, scope);
          {
            const instructions = writeCodexInstructions(GLOBAL_CODEX_SCOPE);
            process.stdout.write(`  codex-instructions: wrote ${instructions.written}\n`);
          }
          break;
        case 'skills':
...
function cmdUninstall(rest, templatesOverride = null) {
  ...
  if (!vals.only && !vals.commands && !vals['codex-agents'] && !vals['codex-skills']) {
  ...
  for (const cls of VALID_CLASSES.filter((c) => phases.includes(c))) {
    ...
    try {
      switch (cls) {
        ...
        case 'codex-skills':
          {
            const instructions = removeCodexInstructions(GLOBAL_CODEX_SCOPE);
            process.stdout.write(`  codex-instructions: removed ${instructions.removed}\n`);
          }
          result = removeCodexSkills(templatesOverride || embeddedTemplates(), scope);
          break;
        case 'skills':
...
export function classifyPath(path, set, options = {}) {
  try {
    if (options.structural ? structuralLeafRejection(path) !== null
      : !lstatSync(path).isFile()) {
      // Non-regular leaves block install; runtime also rejects hard links.
...
function enumerate(scope) {
  ...
  for (const name of AllCodexSkills) {
    rows.push({
      name,
      kind: 'codex-skill',
      state: classifyPath(join(codexSkillsDir, name, SKILL_MANIFEST_LEAF), SetForSkill),
    });
  }
  rows.push({ name: 'AGENTS.md', kind: 'codex-instructions',
    state: classifyCodexInstructions(GLOBAL_CODEX_SCOPE) });
...
function cmdDoctor(rest) {
  ...
  const hasCodexSkills = rows.some((r) => r.kind === 'codex-skill' && r.state !== 'missing');
  const healthRows = rows.filter((r) =>
    (r.kind !== 'codex-agent' || hasCodexAgents)
    && (r.kind !== 'codex-skill' || hasCodexSkills)
    && (r.kind !== 'codex-instructions' || hasCodexSkills));
...
function probeStart(paths) {
  ...
  try {
    ...
    if (e instanceof MalformedSettingsError) {
      process.stderr.write(
        `cah probe start: ${e.message}\n`
        + `  To recover: fix the JSON in ${paths.settingsPath}, then retry start.\n`,
      );
      return 1;
    }
...
function probeStatusReport(paths) {
  ...
  try {
    ...
    process.stderr.write(`cah probe status: ${e.message}\n`);
    return 1;
  }
  process.stdout.write(
...
function cmdVersion() {
  ...
  process.stdout.write(
    `cc-arch-hands ${version} commands=${AllModelCommands.length} agents=${AllModelCommands.length} codex-agents=${AllCodexAgents.length} codex-skills=${AllCodexSkills.length} skills=${AllSkills.length}\n`,
  );
  return 0;
}
