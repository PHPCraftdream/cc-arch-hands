...
import { AllModelCommands, AllCodexAgents, AllSkills, AllAgentTreeSkills, SkillDeps } from './manifest.js';
import { SetForModelCommand, SetForModelAgent, SetForCodexAgent, SetForSkill, SetForAgentTree, SetForBin, classifyContent } from './sentinel.js';
...
import { writeAgentTree, removeAgentTree } from './agent-tree.js';
...
Usage:
  cah install   [--global|--local] [--cwd PATH] [--templates DIR] [--only SELECTOR] [--commands] [--codex-agents] [--agent-tree]
  cah reinstall [--global|--local] [--cwd PATH] [--templates DIR] [--only SELECTOR] [--commands] [--codex-agents] [--agent-tree]
  cah uninstall [--global|--local] [--cwd PATH] [--only SELECTOR] [--commands] [--codex-agents] [--agent-tree]
...
--agent-tree               install/remove only the persistent sub-agent tree skills
                           (agent, agent-new) when used without --only; when
                           combined with --only, also include them.
--only agent-tree          alternative: select the agent-tree skills as a class
                           (opt-in, not part of the default install set).
...
const VALID_CLASSES = ['commands', 'agents', 'codex-agents', 'agent-tree', 'skills', 'bins'];
...
// in the default set.
const OPT_IN_FLAG_CLASSES = ['codex-agents', 'agent-tree', 'commands'];
 ...
 * (commands/agents/skills/bins, plus the opt-in codex-agents/agent-tree) OR
...
// Applies any --<opt-in-class> boolean flags (--codex-agents, --agent-tree).
...
function cmdInstall(rest) {
  ...
  try {
    parsed = parseScopeFlags(rest, {
      ...
      'agent-tree': { type: 'boolean', default: false },
  ...
  let resolved;
  try {
    resolved = resolveDeps(applyOptInFlags(parseOnly(vals.only), vals, vals.only));
  } catch (e) {
    process.stderr.write(`cah install: ${e.message}\n`);
    return 1;
  }
    ...
    try {
      switch (cls) {
        ...
        case 'codex-agents':
          ...
          break;
        case 'agent-tree':
          result = removeAgentTree(embeddedTemplates(), scope);
          break;
        case 'skills':
          result = removeSkills(embeddedTemplates(), scope, { subset: skillsSubset });
          break;
        case 'bins':
...
function enumerate(scope) {
  ...
  for (const name of AllAgentTreeSkills) {
    rows.push({
      name,
      kind: 'agent-tree-skill',
      state: classifyPath(join(skillsDir, name, SKILL_MANIFEST_LEAF), SetForAgentTree),
    });
  }
...
function cmdDoctor(rest) {
  ...
  const hasAgentTree = rows.some((r) => r.kind === 'agent-tree-skill' && r.state !== 'missing');
  ...
  const healthRows = rows.filter((r) =>
    ...
    && (r.kind !== 'agent-tree-skill' || hasAgentTree)
...
function cmdVersion() {
  ...
  process.stdout.write(
    `cc-arch-hands ${version} commands=${AllModelCommands.length} agents=${AllModelCommands.length} codex-agents=${AllCodexAgents.length} skills=${AllSkills.length} agent-tree=${AllAgentTreeSkills.length}\n`,
  );
  return 0;
}
