...
import { acquireLease, releaseLease, renewLease } from './lease-lock.js';
...
function validCacheRecord(cache) {
  return cache && (cache.latestVersion === null || validVersion(cache.latestVersion) !== null);
}

function cacheIsFresh(cache, nowMs, ttlMs) {
  return validCacheRecord(cache) && Number.isFinite(cache.checkedAt)
    && cache.checkedAt <= nowMs + FUTURE_SKEW_TOLERANCE_MS
...
function fetchLatestVersionSync(timeoutMs, fetchOverride = null) {
  ...
  try {
    ...
    return validVersion(obj.version);
  } catch {
    return null;
  }
}
...
// prefix ranking higher; build metadata (+...) is ignored entirely. Core
// numeric identifiers follow the strict grammar of #spec-item-2 (`0`, or a
// digit sequence not starting with `0`), and a prerelease identifier that is
// all digits with a leading zero is invalid per #spec-item-9: the whole
// version is rejected rather than silently normalized, so malformed input
// can never report a false "update available".
...
export function getLatestVersion(cachePath, ttlMs = DEFAULT_TTL_MS, nowMs = Date.now(), options = {}) {
  ...
  if (cacheIsFresh(cached, nowMs, ttlMs)) {
    return validVersion(cached.latestVersion);
  }

  const lock = acquireRefreshLock(cachePath, options.testInterlock);
  if (!lock) {
    const afterWait = readJsonMaybe(cachePath);
    return validVersion(afterWait?.latestVersion) || validVersion(cached?.latestVersion);
  }
  ...
  try {
    ...
    if (cacheIsFresh(afterLock, nowMs, ttlMs)) {
      return validVersion(afterLock.latestVersion);
    }
    ...
    const changedToFresher = cacheRecordChanged(cached, beforePublish)
      && validCacheRecord(beforePublish)
      && Number.isFinite(beforePublish.checkedAt)
      && beforePublish.checkedAt <= nowMs + FUTURE_SKEW_TOLERANCE_MS
    ...
    if (changedToFresher) {
      return validVersion(beforePublish.latestVersion);
    }

    const latestVersion = fetched || validVersion(beforePublish?.latestVersion)
      || validVersion(cached?.latestVersion);
    ...
    const published = writeJsonAtomic(
      ...
      { expectedDestination: beforePublishSnapshot.expectedDestination,
        assertOwnership: () => renewLease(lock), lifecycleLease: lock,
        testInterlock: options.testInterlock },
    );
    if (!published) {
      const concurrentWinner = readJsonMaybe(cachePath);
      const winner = validVersion(concurrentWinner?.latestVersion);
