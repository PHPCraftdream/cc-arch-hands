...
function cacheLeafSnapshot(cachePath) {
  try {
    return { exists: true, identity: lstatSync(cachePath, { bigint: true }) };
  } catch (e) {
    if (e && e.code === 'ENOENT') return { exists: false, identity: null };
    return { exists: true, identity: null };
  }
}
...
export function getLatestVersion(cachePath, ttlMs = DEFAULT_TTL_MS, nowMs = Date.now()) {
  ...
  try {
    ...
    const expectedDestination = cacheLeafSnapshot(cachePath);
