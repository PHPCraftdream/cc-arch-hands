...
// A live owner means another caller is already doing the best-effort refresh.
// Hook callers should return their stale value instead of synchronously
// waiting for that refresh. This short window is only for lock publication
// and stale-lock recovery races.
const LOCK_RECOVERY_WAIT_MS = 250;
...
function acquireRefreshLock(cachePath) {
  ...
  const deadline = Date.now() + LOCK_RECOVERY_WAIT_MS;
  ...
  while (Date.now() < deadline) {
    try {
      ...
      // A valid live owner is already refreshing the cache. Returning here
      // keeps status/Stop hooks on the stale-cache fast path; waiting for the
      // owner would make an otherwise best-effort check block for seconds.
    ...
    sleepSync(Math.min(10, remaining));
  }
  return null;
}
