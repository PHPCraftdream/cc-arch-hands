...
function quarantineUnexpectedFence(fencePath, ownerFile) {
  try {
    ...
    if (!entries.some((entry) => entry !== ownerFile)) return false;
    // Preserve foreign contents in one deterministic, bounded namespace. The
    // source fence name is already unique for its operation, so no retry
    // suffix is needed and the destination cannot be mistaken for an active
    // `.taken-*`/`.stale-*` fence.
    const quarantineDir = join(dirname(fencePath), LEASE_QUARANTINE_DIR);
    mkdirSync(quarantineDir, { recursive: true });
    renameSync(fencePath, join(quarantineDir, basename(fencePath)));
    return true;
  } catch {
    return false;
  }
