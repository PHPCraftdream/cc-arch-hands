...
import { leaseExpired, LEASE_MAX_MS, ownerTimestamp, sleepSyncWait as sleepSync } from './lease-clock.js';

export { LEASE_MAX_MS, ownerTimestamp } from './lease-clock.js';
...
// One releaseLease() races at most RELEASE_TOTAL_WAIT_MS of wall clock, split
// into three sequential shares so no phase can multiply or starve another:
// the fenced rename, ONE genuine removal attempt (transient rejections are
// retried in place until the share expires), and terminal disposal.
const RELEASE_FENCE_WAIT_MS = 750;
const RELEASE_REMOVAL_WAIT_MS = 250;
const RELEASE_DISPOSAL_WAIT_MS = 250;
export const RELEASE_TOTAL_WAIT_MS =
  RELEASE_FENCE_WAIT_MS + RELEASE_REMOVAL_WAIT_MS + RELEASE_DISPOSAL_WAIT_MS;
...
function testInterlock(phase, stage, options = null) {
  if (typeof options?.testInterlock === 'function') options.testInterlock(phase, stage);
}
...
// competing reclaimers; a destination occupied up front or by this call's
// own partial move falls through to the slots, and an emptied root is removed.
function quarantineFenceDir(fencePath, deadline) {
  try {
    ...
    try { if (readdirSync(quarantineRoot).length === 0) rmdirSync(quarantineRoot); } catch { /* raced */ }
    return false;
  } catch {
    return false;
  }
...
function recoverFence(path, fencePath, options) {
  ...
  if (currentIdentity) {
    ...
    // Same guarded leaseExpired() rule as claimExpired() (below): a
    // future-dated heartbeat after a clock rollback must read as expired
    // here too, or this displacement check and the reclaim decision for the
    // same lease state disagree again, just one call site over.
    const currentLeaseExpired = currentOwner
      && (!pidIsAlive(currentOwner.pid)
        || (ownerTimestamp(currentOwner) !== null
          && leaseExpired(currentOwner, nowMs, maxLeaseMs)));
    ...
    if (!sameOwnerSnapshot(quarantine, currentSnapshot, readOwner)) {
      // The displacement no longer matches what we moved: never destroy it,
      // and never leave it in the unbounded .abandoned- namespace — restore
      // it if possible, otherwise park it in the fixed reported quarantine.
      if (!restoreWithoutOverwrite(quarantine, path, ownerFile, recoveryDeadline)) {
        quarantineFenceDir(quarantine, recoveryDeadline);
      }
      return false;
    }
    // Restore the old fence before deleting the displaced claim. If the
    // restore loses to a successor, the displaced claim is still spent
    // state: dispose it so the .abandoned- namespace cannot grow unbounded.
    if (!restoreWithoutOverwrite(fencePath, path, ownerFile, recoveryDeadline)) {
      disposeOwnFence(quarantine, ownerFile, recoveryDeadline);
      return false;
    }
...
function hasInFlightFence(path, options, ignoredFence = null) {
  const suffixes = new Set([options.fenceSuffix, ...(options.legacyFenceSuffixes || [])]);
  let blocked = false;
  for (const suffix of suffixes) {
    const recovery = suffix === options.fenceSuffix ? options : { ...options, fenceSuffix: suffix };
    for (const fencePath of fencePaths(path, suffix)) {
      if (fencePath === ignoredFence) continue;
      if (!recoverFence(path, fencePath, recovery)) blocked = true;
    }
  }
  return blocked || [...suffixes].some((suffix) =>
    fencePaths(path, suffix).some((fence) => fence !== ignoredFence));
}

function claimExpired(snapshot, options) {
  if (!snapshot.identity) return false;
  if (snapshot.owner) {
    // Reclaim and renewal share expiry; an unknown live heartbeat is preserved.
    if (!options.pidIsAlive(snapshot.owner.pid)) return true;
    const timestamp = ownerTimestamp(snapshot.owner);
    return timestamp !== null && leaseExpired(snapshot.owner, options.nowMs, options.maxLeaseMs);
  }
...
export function acquireLease(path, options = {}) {
  ...
  const config = {
    ...
    legacyFenceSuffixes: Array.isArray(options.legacyFenceSuffixes)
      ? options.legacyFenceSuffixes.filter((suffix) => typeof suffix === 'string' && suffix) : [],
  ...
  // One claim attempt, plus one after reclaim; elapsed I/O cannot skip either.
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (!error || error.code !== 'EEXIST' || attempt === 1) return null;
      if (hasInFlightFence(path, config)) return null;
      ...
      testInterlock(config.interlockPhase, 'claim-removal', config);
      if (removeClaimPath(fence.path, ownerFile, config.recoveryDeadline)) continue;
      // Expired claim + this process's own takeFence creation: dispose rather
      // than strand a `.taken-` entry that would block every later acquire.
      // Deferred vs releaseLease(): the disposal's boolean goes unobserved and
      // success can still hit the outer recoveryDeadline; both self-converge.
      disposeOwnFence(fence.path, ownerFile, Date.now() + LEASE_RECOVERY_WAIT_MS);
      continue;
    }
  }
  return null;
...
export function renewLease(lease, nowOrOptions = Date.now()) {
  ...
  if (leaseExpired(current, nowMs, maxLeaseMs)) return false;
...
// before the rename returned. Dispose outright - releaseLease()'s own spent
// claim, or the reclaim's expired foreign one - or quarantine a stray entry.
...
export function releaseLease(lease) {
  ...
  // Release computes fresh phase budgets here, each an explicit share of
  // RELEASE_TOTAL_WAIT_MS, so no phase can multiply or silently starve
  // another: the fence rename, the removal attempt, the terminal disposal.
  const releaseOptions = {
    ...options,
    interlockPhase: options.releaseInterlockPhase || 'lease-release',
    recoveryDeadline: Date.now() + RELEASE_FENCE_WAIT_MS,
  };
  ...
  // Removal and disposal must resume from fence.path. ONE genuine attempt:
  // removeClaimPath retries transient rejections in place until its share
  // expires, and a non-transient rejection (the stray-entry guard) changes no
  // state between calls, so a wider attempt loop only raced third parties.
  const removalDeadline = Date.now() + RELEASE_REMOVAL_WAIT_MS;
  ...
  // A rejected removal leaves a fence this process provably created; disposal
  // is unconditional and gets a fresh share after a possibly-spent attempt.
  return disposeOwnFence(fence.path, options.ownerFile, Date.now() + RELEASE_DISPOSAL_WAIT_MS);
}
