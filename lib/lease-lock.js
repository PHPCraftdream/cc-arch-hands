...
function claimExpired(snapshot, options) {
  ...
  if (snapshot.owner) {
    // A dead owner is immediately reclaimable regardless of its heartbeat. A
    // live owner's claim expires by the SAME timestamped rule renewLease()
    // applies (lease-clock.js): a future-dated heartbeat after a clock
    // rollback, or one older than the lease period, expires for reclaim
    // exactly as it does for renewal, so a lease can never be stuck
    // unrenewable by its owner yet untakeable by anyone else. A live owner
    // with NO recorded heartbeat stays non-expiring evidence: never steal a
    // claim that cannot be validated against the shared expiry rule.
