...
const directoryScanStats = { enabled: false, lookaheadCalls: 0, visited: 0, inspections: 0 };
...
function interlockDeadline() {
  return Date.now() + LEASE_RECOVERY_WAIT_MS;
}
...
function testInterlock(phase, stage, deadline, options = null) {
...
function takeFence(path, expected, options) {
  ...
  testInterlock(options.interlockPhase, 'before', options.interlockDeadline, options);
  const moved = withTransientRetry(
    () => renameSync(path, fencePath),
    options.recoveryDeadline,
  );
  if (!moved.ok) return { path: null, error: moved.error };
  testInterlock(options.interlockPhase, 'vacancy', options.interlockDeadline, options);
...
export function acquireLease(path, options = {}) {
  ...
  while (Date.now() < config.recoveryDeadline) {
    ...
    try {
      mkdirSync(path);
      testInterlock('lease-owner-write', 'before', config.interlockDeadline, config);
...
export const heartbeatLease = renewLease;
...
export function releaseLease(lease) {
  ...
  testInterlock(releaseOptions.interlockPhase, 'before', releaseOptions.interlockDeadline, releaseOptions);
...
export function removePathIfUnchangedRecoverable(
  ...
  testInterlock(phase, 'before', interlockDeadline(), options);
...
export function removePathIfUnchanged(path, expectedIdentity, phase = 'lease-remove', options = {}) {
  return removePathIfUnchangedRecoverable(path, expectedIdentity, phase, options).ok;
}
