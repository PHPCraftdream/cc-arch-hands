...
export function readTranscriptStats(transcriptPath) {
  ...
  // pair, the missing field may live in an earlier entry — re-scan the whole
  // file. Small transcripts read fully on the first pass (partial === false).
...
function scanTranscript(raw) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (usedTokens === null && obj.type !== 'user') {
      // Skip user entries: tool results live there, and an MCP tool that echoes
      // an upstream usage object would otherwise be mistaken for session usage.
