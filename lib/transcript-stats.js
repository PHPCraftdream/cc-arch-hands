 ...
 * All three fields are taken from one newest context-bearing assistant turn.
...
export function readTranscriptStats(transcriptPath, options = {}) {
  ...
  // Only missing mandatory usage/model data triggers this fallback.
  // A missing optional requestId is not a reason to read the transcript
  // prefix; only the mandatory usage/model fields trigger this fallback.
  if (tail.partial && (result === null || result.usedTokens === null || result.modelId === null)) {
    try {
      const readWholeFile = typeof options.readWholeFile === 'function'
        ? options.readWholeFile
        : (path) => readFileSync(path, 'utf8');
      result = scanTranscript(readWholeFile(transcriptPath));
    } catch {
...
function scanTranscript(raw) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (obj.type === 'user' || !isAssistantEntry(obj)) continue;

    const entryRequestId = findRequestId(obj);
    const entryUsedTokens = findContextTokens(obj);
    const entryModelId = findModel(obj);

    if (result === null) {
      if (entryUsedTokens === null && entryModelId === null) continue;
      result = {
        usedTokens: entryUsedTokens,
        modelId: entryModelId,
        requestId: entryRequestId,
      };

      // Without a request id there is no safe way to prove an adjacent entry
      // belongs to this turn. Return only the anchor record's fields.
      if (entryRequestId === null) return result;
    } else {
      // Assistant entries of another (or unidentified) request are a hard turn
      // boundary. Never search past one for a coincidentally repeated id.
      if (entryRequestId !== result.requestId) break;
      if (result.usedTokens === null && entryUsedTokens !== null) {
        result.usedTokens = entryUsedTokens;
      }
      if (result.modelId === null && entryModelId !== null) {
        result.modelId = entryModelId;
      }
    }

    if (result.usedTokens !== null && result.modelId !== null) return result;
  }
...
function isAssistantEntry(obj) {
  ...
  return obj.type == null && (findContextTokens(obj) !== null || findModel(obj) !== null);
}
...
function writeJsonAtomic(path, value) {
  ...
  try {
    ...
    rateCacheWriteCounter = (rateCacheWriteCounter + 1) % 1_000_000;
    tmp = `${path}.${process.pid}.${Date.now()}.${rateCacheWriteCounter}.tmp`;
...
function pruneRateContextSidecars(cachePath, nowMs) {
  ...
  for (const name of names) {
    ...
    try {
      ...
      if (nowMs - stat.mtimeMs > RATE_LIMITS_MAX_AGE_MS) {
        unlinkSync(sidecar);
        continue;
      }
...
export function persistRateLimitsCache(
  ...
  if (hasRateState || !existing) {
    const global = {
      version: 2,
      fiveHour: fiveHour || (existing && existing.fiveHour) || null,
      sevenDay: sevenDay || (existing && existing.sevenDay) || null,
