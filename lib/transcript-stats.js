...
function publishMigratedContext(sourcePath, targetPath, source, targetIdentity) {
  try {
    writeFileAtomic(targetPath, source.raw, {
      expectedDestination: targetIdentity || null,
    });
    try { removeOwnedRegularFile(sourcePath, source.identity); } catch { /* retry next call */ }
    return true;
  } catch {
    return false;
  }
...
function migrateLegacyRateContext(cachePath, sessionId, nowMs) {
  ...
  if (!usableContext(target, nowMs) || legacyContextIsNewer(source, target)) {
    publishMigratedContext(sourcePath, targetPath, source, target.identity);
  } else {
    try { removeOwnedRegularFile(sourcePath, source.identity); } catch { /* retry next call */ }
  }
}
