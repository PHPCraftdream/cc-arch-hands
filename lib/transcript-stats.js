...
export function rateLimitsContextPath(cachePath, sessionId) {
  return join(dirname(cachePath), RATE_CONTEXT_NAMESPACE, `${rateContextHash(sessionId)}.json`);
}
...
function migrateLegacyRateContext(cachePath, sessionId) {
  ...
  if (legacyContextIsNewer(source, target)) {
...
function readContextSidecar(cachePath, sessionId, nowMs) {
  migrateLegacyRateContext(cachePath, sessionId);
  ...
  const candidates = [current, legacy].filter((entry) => entry && isFresh(entry.value?.capturedAt, nowMs));
  candidates.sort((a, b) => {
...
function pruneRateContextSidecars(cachePath, nowMs, testInterlock = null) {
  const namespace = join(dirname(cachePath), RATE_CONTEXT_NAMESPACE);
  let handle;
  try {
