...
function semanticFreshness(path, inspected) {
  ...
  return {
    ...
    // Keep filesystem nanoseconds as BigInt through collision ordering. They
    // are already beyond Number's exact range on ordinary modern filesystems;
    // conversion is reserved for isOlderThan's bounded age check.
...
function transactionInspection(markerDir, { reconcileEmpty = false } = {}) {
  ...
  if (state.status === ABSENT && reconcileEmpty) {
    try {
      if (readdirSync(capacityTransactionPath(markerDir)).length === 0) {
        rmdirSync(capacityTransactionPath(markerDir));
...
// A process can die after creating the private staging directory but before
// renaming it into the canonical transaction path. Keep this maintenance
// bounded because the parent is shared cache space, and only touch the exact
// shape created by prepareCapacityEviction. A complete staged transaction is
// promoted so the normal recovery proof can decide whether its victim belongs
// back in place; an empty stage is simply the pre-write crash boundary.
function reconcileCapacityStages(markerDir, cfg) {
  ...
  const scan = streamDirectoryEntries(dirname(markerDir), cfg.scanCap, (entry) => {
    if (!entry.name.startsWith(stagePrefix) || !entry.isDirectory()) return;
    const stagePath = join(dirname(markerDir), entry.name);
    const entries = [];
    const stageScan = streamDirectoryEntries(stagePath, 2, (stageEntry) => {
      entries.push(stageEntry.name);
    });
    if (!stageScan.complete) { indeterminate = true; return; }
    if (entries.length === 0) {
    ...
    if (!parseTransactionState(stagedState.content)) {
      // A partial or foreign state is not a recoverable transaction. Keep it
      // so an operator can inspect it; the indeterminate result bounds the
      // maintenance pass without deleting unproved data.
      indeterminate = true;
      return;
    }
  ...
  // An overflow in the shared parent is intentionally only a bounded pass;
  // it must not block current-session work merely because unrelated entries
  // follow the first cap. Any matching stage we did inspect remains strict.
  return !indeterminate && (scan.complete || scan.visited > 0);
}
...
function recoverCapacityTransaction(markerDir, state, capacityLease, nowMs, cfg) {
...
function prepareCapacityEviction(markerDir, marker, markerBefore, nonce, nowMs, cfg, context,
  ...
  if (!reconcileCapacityStages(markerDir, cfg)) return null;
  const tx = readTransaction(markerDir, { reconcileEmpty: true });
  ...
  try {
    ...
    const stageDir = join(dirname(txDir), `.${basename(txDir)}-stage-${randomUUID()}`);
    mkdirSync(stageDir, { mode: 0o700 });
    try {
      writeFileSync(join(stageDir, 'transaction.json'), JSON.stringify(state) + '\n', { flag: 'wx', mode: 0o600 });
      renameSync(stageDir, txDir);
    } catch (error) {
      try { rmdirSync(stageDir); } catch { /* preserve non-empty or indeterminate stage */ }
      throw error;
    }
...
export function migrateMarkerState(options) {
  ...
  try {
    if (!reconcileCapacityStages(markerDir, cfg)) return { blocked: true };
    const transaction = readTransaction(markerDir, { reconcileEmpty: true });
    if (transaction === null
...
export function claimMarker(options) {
  ...
  if (!reconcileCapacityStages(markerDir, cfg)) {
