...
function statePathsBelongToMarkerDir(markerDir, state) {
  ...
  return basename(state.capacityLeasePath).startsWith('.cah-marker-capacity-');
}
const {
...
function reconcileCapacityStages(markerDir, cfg, capacityLease) {
  return reconcileCapacityStageArtifacts(markerDir, cfg, {
    ...
    validateState: (candidate) => statePathsBelongToMarkerDir(markerDir, candidate),
  });
}
...
export function migrateMarkerState(options) {
  ...
  if (readTransaction(markerDir, { preflight: true }) === null) return { blocked: true };
  ...
  try {
    ...
    const transaction = readTransaction(markerDir, { reconcileEmpty: true, recoverPublication: true,
      ...
      lifecycleLeases: [migrationLease], retirementLimit: cfg.retirementBatchCap });
    if (transaction === null
...
export function claimMarker(options) {
  ...
  if (readTransaction(markerDir, { preflight: true }) === null) return null;
  ...
  const tx = readTransaction(markerDir, { reconcileEmpty: true, recoverPublication: true,
    ...
    retirementLimit: cfg.retirementBatchCap });
