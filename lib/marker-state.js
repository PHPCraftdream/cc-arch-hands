...
function acquireCapacityLeaseForPrune(markerDir, cfg, nowMs) {
  ...
  // A hook prunes once more while its own marker claim is still settling. In
  // that synchronous same-process case the capacity lease is already ours;
  // borrow its exact persisted generation instead of trying to reacquire (or
  // releasing) the live lease. A different process remains fenced out.
