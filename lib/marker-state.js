...
// A failed release leaves a live-owner lease (or an in-flight fence) on the
// exact path the caller re-acquires next, silently dropping the turn's stamp.
// Every releaseLease attempt carries its own fresh recovery deadline, so a
// bounded retry rides out transient contention instead of abandoning the
// lease after one exhausted budget.
