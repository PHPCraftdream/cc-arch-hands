...
function mutateVictimSlot(markerDir, state, context, sourcePath, destinationPath, destinationKey,
  mutation, { interlock = true } = {}) {
  ...
  if (source.status !== PRESENT || !source.identity?.isFile
      || identityKey(source.identity) !== state.victimKey) {
    if (context) context.victimMismatch = { path: sourcePath, expected: state.victimKey,
      actual: source.status === PRESENT ? identityKey(source.identity) : source.status };
    return false;
  }
  if (destinationKey !== undefined) {
    ...
    if (!matches) {
      if (context) context.victimMismatch = { path: destinationPath, expected: destinationKey,
        actual: destination.status === PRESENT ? identityKey(destination.identity) : destination.status };
      return false;
    }
  }
  try { mutation(); return true; } catch { return false; }
}
...
function restoreVictim(markerDir, state, context) {
  ...
  if (canonical.status === ABSENT) {
    return mutateVictimSlot(markerDir, state, context, capacitySlotPath(markerDir), state.victim,
      null, () => renameSync(capacitySlotPath(markerDir), state.victim));
  }
  ...
  return mutateVictimSlot(markerDir, state, context, capacitySlotPath(markerDir), state.victim,
    identityKey(canonical.identity), () => unlinkSync(capacitySlotPath(markerDir)));
}
  ...
  try {
    if (slot.status === ABSENT) {
      ...
      if (victim.status === PRESENT) {
        if (!mutateVictimSlot(markerDir, state, context, state.victim,
          capacitySlotPath(markerDir), null,
          () => renameSync(state.victim, capacitySlotPath(markerDir)))) return false;
      }
    }
    ...
    if (finalSlot.status === PRESENT) {
      if (!mutateVictimSlot(markerDir, state, context, capacitySlotPath(markerDir), null,
        undefined, () => unlinkSync(capacitySlotPath(markerDir)))) return false;
    }
...
function prepareCapacityEviction(markerDir, marker, markerBefore, nonce, nowMs, cfg, context,
  ...
  try {
    ...
    if (!mutateVictimSlot(markerDir, state, context, scan.oldest.path,
      capacitySlotPath(markerDir), null,
      () => renameSync(scan.oldest.path, capacitySlotPath(markerDir)), { interlock: false })) {
...
export function claimMarker(options) {
  ...
  const recoveredContext = { capacityLease, recovered: true, testInterlock: cfg.testInterlock };
  if (tx) {
  ...
  const context = { capacityLease, sessionClaim, testInterlock: cfg.testInterlock };
