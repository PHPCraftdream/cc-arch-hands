...
import { reconcileCapacityStages as reconcileCapacityStageArtifacts } from './marker-capacity-stage.js';
...
// One deterministic staging slot is enough because the capacity lease
// serializes transactions for the marker namespace.
...
function transactionInspection(markerDir, { reconcileEmpty = false } = {}) {
  ...
  if (state.status === ABSENT) {
    // fs-atomic fences the state leaf itself while it performs its final
    // no-overwrite publication. Treat the exact displaced state as the live
    // transaction during that short generation-fenced interval.
