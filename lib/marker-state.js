...
import {
  ...
  syncBestEffort, victimFencePath, victimIdentityKey,
...
function capacityOps() {
  return { transactionStillCurrent, inspectPath, identityKey, present: PRESENT, absent: ABSENT };
}
...
function cleanupTransaction(markerDir, state, context) {
  ...
  try {
    ...
    if (dir.status === PRESENT) {
      if (readdirSync(capacityTransactionPath(markerDir)).length !== 0) return false;
      rmdirSync(capacityTransactionPath(markerDir));
      syncBestEffort(dirname(capacityTransactionPath(markerDir)));
    }
    return true;
  } catch { return false; }
}
function restoreVictim(markerDir, state, context) {
  const canonicalFence = victimFencePath(state.victim);
  const canonicalFenceState = inspectPath(canonicalFence);
  if (canonicalFenceState.status === INDETERMINATE) return false;
  if (canonicalFenceState.status === PRESENT
      && !recoverVictimFence(markerDir, state, context, state.victim, capacityOps())) return false;
...
export function finishCapacityEviction(markerDir, state, context) {
  ...
  try {
    if (!recoverVictimFence(markerDir, state, context, capacitySlotPath(markerDir), capacityOps())) {
      return false;
    }
