...
function publishedLeafMatches(path, snapshot) {
  try {
    assertExactFileSnapshot(path, snapshot);
    return true;
  } catch {
    return false;
  }
  ...
  return withProbeLease(settingsPath, () => {
    ...
    try {
      ...
      const settingsReverted = settingsCommitted
        ...
        : false;
      // The transition's own backup is removed unless the settings side is a
      // committed publication that is still exactly ours and could not be
      // reverted: only that combination leaves an armed probe whose backup is
      // its only recovery path. A successor that replaced the settings leaf
      // after the rename owns the state and does not need this backup.
      const settingsLeafStillOurs = settingsCommitted
        && publishedLeafMatches(settingsPath, settingsCommitted);
      if (settingsReverted || !settingsLeafStillOurs) {
        rollbackExactPublication(
