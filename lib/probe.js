...
function assertExactFileSnapshot(path, expectedSnapshot) {
  ...
  const currentIdentity = regularFileIdentity(path);
...
function capturePublishedSnapshot(path, payload) {
  const snapshot = captureRegularFileSnapshot(path);
  if (!snapshot.present || !snapshot.expectedDestination.identity
      || !snapshot.content.equals(Buffer.from(payload))) return null;
  return snapshot;
}
...
export function enableProbe({ settingsPath, probeBinAbsPath, backupPath, logPath }) {
  return withProbeLease(settingsPath, () => {
    ...
    const backupPayload = JSON.stringify({ previous: settings.statusLine || null }, null, 2) + '\n';
    ...
    try {
      ...
      writtenBackupSnapshot = capturePublishedSnapshot(backupPath, backupPayload);
      if (!writtenBackupSnapshot) throw new BackupChangedError(backupPath);
      ...
      assertBackupUnchanged(backupPath, writtenBackupSnapshot);
      settingsPayload = writeJsonAtomic(
        settingsPath,
        settings,
        indent,
        settingsSnapshot.expectedDestination,
      );
      publishedSettingsSnapshot = capturePublishedSnapshot(settingsPath, settingsPayload);
      if (!publishedSettingsSnapshot) throw settingsPublicationError();
      waitForProbeTestInterlock(
      ...
      // If writeFileAtomic published and then reported an error, discover the
      // candidate by its exact bytes. A foreign successor will not match and
      // is consequently never rolled back.
      if (!publishedSettingsSnapshot && settingsPayload) {
        try { publishedSettingsSnapshot = capturePublishedSnapshot(settingsPath, settingsPayload); } catch { /* preserve error */ }
      }
      if (!writtenBackupSnapshot) {
        try { writtenBackupSnapshot = capturePublishedSnapshot(backupPath, backupPayload); } catch { /* preserve error */ }
      }
...
export function disableProbe({ settingsPath, backupPath }) {
  return withProbeLease(settingsPath, () => {
    ...
    const restoredPayload = JSON.stringify(settings, null, indent) + '\n';
    let publishedSettingsSnapshot = null;
    try {
      writeJsonAtomic(settingsPath, settings, indent, settingsSnapshot.expectedDestination);
      publishedSettingsSnapshot = capturePublishedSnapshot(settingsPath, restoredPayload);
      if (!publishedSettingsSnapshot) throw settingsPublicationError();
      waitForProbeTestInterlock(
      ...
      if (!publishedSettingsSnapshot) {
        try { publishedSettingsSnapshot = capturePublishedSnapshot(settingsPath, restoredPayload); } catch { /* preserve error */ }
      }
      rollbackExactPublication(settingsPath, settingsSnapshot, publishedSettingsSnapshot);
      throw error;
    }
