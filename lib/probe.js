...
import { acquireSettingsLock, releaseSettingsLock, renewSettingsLock } from './settings-lock.js';
...
function writeJsonAtomic(path, data, indent, expectedDestination, lease, testInterlock = null) {
  const payload = JSON.stringify(data, null, indent) + '\n';
  return writeFileAtomic(path, payload, {
    expectedDestination,
    ...probeMutationOptions(lease, testInterlock),
  });
}
...
export class ProbeLeaseLostError extends Error {
  constructor(cause) {
    super('probe settings lease was lost mid-transition; refusing to change settings, backup, or log', { cause });
    this.name = 'ProbeLeaseLostError';
    this.code = 'ERR_PROBE_LEASE_LOST';
    this.leaseLost = true;
    this.cause = cause;
  }
}
...
function assertProbeLeaseHeld(handle, cause) {
  if (!renewSettingsLock(handle)) throw new ProbeLeaseLostError(cause);
}

function probeMutationOptions(lease, testInterlock) {
  return { assertOwnership: () => assertProbeLeaseHeld(lease),
    lifecycleLease: lease.lease, testInterlock };
}
...
// alone.
function rollbackExactPublication(path, beforeSnapshot, publishedSnapshot, lease,
  testInterlock = null, assertRecovery = null) {
  if (!publishedSnapshot) return false;
  const options = probeMutationOptions(lease, testInterlock);
  const assertRollback = () => { options.assertOwnership(); assertRecovery?.(); };
  try {
    assertRollback();
    ...
    if (beforeSnapshot.expectedDestination.identity === null) {
      return removeOwnedRegularFile(path, publishedSnapshot.expectedDestination,
        { ...options, assertOwnership: assertRollback });
    }
    ...
    if (error instanceof ProbeLeaseLostError) throw error;
    return false;
  }
}
...
function removeExactBackup(backupPath, snapshot, lease, testInterlock) {
...
export function enableProbe({ settingsPath, probeBinAbsPath, backupPath, logPath }, options = {}) {
  return withProbeLease(settingsPath, (lease) => {
    ...
    writeFileAtomic(logPath, '', {
      ...probeMutationOptions(lease, options.testInterlock),
      expectedDestination: logSnapshot.expectedDestination,
    });
    ...
    try {
      writtenBackupSnapshot = writeJsonAtomic(
        ...
        2, backupSnapshot.expectedDestination, lease, options.testInterlock,
      );
      ...
      publishedSettingsSnapshot = writeJsonAtomic(
        settingsPath,
        settings,
        indent, settingsSnapshot.expectedDestination, lease, options.testInterlock,
      );
      options.testInterlock?.(
      ...
      assertProbeLeaseHeld(lease, error);
      ...
      const settingsReverted = settingsCommitted
        ? rollbackExactPublication(
          settingsPath, settingsSnapshot, settingsCommitted, lease, options.testInterlock,
        )
        : false;
...
export function disableProbe({ settingsPath, backupPath }, options = {}) {
  return withProbeLease(settingsPath, (lease) => {
    ...
    try {
      assertProbeLeaseHeld(lease);
      publishedSettingsSnapshot = writeJsonAtomic(
        settingsPath, settings, indent, settingsSnapshot.expectedDestination, lease, options.testInterlock,
      );
      options.testInterlock?.(
        'disable-post-settings-rename', 'disable-after-settings-rename', 'post-settings-rename',
      );

      // Preserve successor settings and backups when either postcheck fails.
      try {
        assertExactFileSnapshot(settingsPath, publishedSettingsSnapshot);
      } catch {
        throw settingsPublicationError();
      ...
      // the lease — not even its own earlier stages.
      assertProbeLeaseHeld(lease, error);
      // Reactivate a failed stop only while its verified backup remains usable.
      const settingsCommitted = publishedSettingsSnapshot
        || committedPublicationSnapshot(settingsPath, error);
    ...
    if (backupSnapshot.expectedDestination.identity) {
      removeExactBackup(backupPath, backupSnapshot, lease, options.testInterlock);
    }
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    try {
      ...
      if (record && typeof record === 'object' && !Array.isArray(record)
          && typeof record.raw === 'string') out.push(record);
    } catch {
      // skip malformed line
    }
  }
