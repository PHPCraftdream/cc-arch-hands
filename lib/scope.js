...
export class Scope {
  ...
  codexRoot() {
    // `--local` is a strict operation over an existing Claude project. A
    // Codex-only selection must obey the same guard even though its files live
    // below .codex rather than .claude.
    if (!this.global && this.strict) this.claudeRoot();
    if (this.global) {
  ...
  resolveCodexSkillsDir() {
    return join(this.codexRoot(), SKILLS_SUBDIR);
  }
