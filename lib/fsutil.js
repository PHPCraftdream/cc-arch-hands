...
export function sameFileIdentity(left, right) {
  ...
  if (!sameStatInteger(left.dev, right.dev) || !sameStatInteger(left.ino, right.ino)) {
    return false;
  }
  ...
  return sameStatInteger(left.size, right.size)
    && sameMtime(left, right);
}

// All identities produced by this module use bigint stats. The narrow
// numeric fallback keeps older callers that still pass a default fs.Stats
// identity interoperable while the exact path remains a BigInt comparison.
function sameStatInteger(left, right) {
...
function sameMtime(left, right) {
  if (typeof left.mtimeNs === 'bigint' && typeof right.mtimeNs === 'bigint') {
    return left.mtimeNs === right.mtimeNs;
  }
  if (typeof left.mtimeNs === 'bigint' && typeof right.mtimeMs === 'number') {
    return Number(left.mtimeNs / 1_000_000n) === Math.floor(right.mtimeMs);
  }
  if (typeof right.mtimeNs === 'bigint' && typeof left.mtimeMs === 'number') {
    return Number(right.mtimeNs / 1_000_000n) === Math.floor(left.mtimeMs);
  }
  // Compatibility for legacy numeric fs.Stats passed by older consumers.
