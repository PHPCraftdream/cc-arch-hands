...
export function removeOwnedRegularFile(path, expected, options = {}) {
  ...
  const quarantine = deterministicQuarantinePath(path);
  const occupied = quarantineEntry(quarantine);
  if (occupied.exists) {
    // A stranded slot may be reclaimed only when its inode is provably the
    // exact managed inode this call was asked to remove. Foreign data,
    // including a symlink or directory, is never auto-deleted.
    if (occupied.info && sameFileIdentity(occupied.info, expected)) {
      unlinkForRemoval(quarantine);
      if (!sameFileIdentity(regularFileIdentity(path), expected)) return true;
    } else {
      return preservedRemoval(quarantine);
    }
  }
  ...
  try {
    ...
    if (moved === 'preserved') return preservedRemoval(quarantine);
...
function renameForRemoval(from, to, expected, parentIdentities) {
  ...
  for (let attempt = 0; attempt < RENAME_RETRIES; attempt++) {
    ...
    if (quarantineEntry(to).exists) return 'preserved';
...
function quarantineEntry(path) {
  try {
    const info = lstatSync(path);
    return { exists: true, info: info.isFile() ? info : null };
  } catch (e) {
    if (e.code !== 'ENOENT') throw e;
...
function preservedRemoval(quarantinePath) {
  return {
    removed: false,
...
export function pruneOrphans(dir, knownNames, sentinelSet, options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (ownership === Ownership.mine || ownership === Ownership.legacy) {
      ...
      if (result === true) pruned++;
      else if (result?.quarantinePath && options.onPreserved) {
        options.onPreserved(result.quarantinePath);
      }
    }
  }
  return pruned;
...
export function pruneOrphanDirs(root, knownNames, manifestLeaf, sentinelSet) {
  ...
  for (const entry of entries) {
    ...
    if (children.length !== 1) {
      ...
      const quarantineLeaf = `${manifestLeaf}.cah-owned-remove`;
      if (children.some((child) => child.name === quarantineLeaf)) {
        preserved.push(relativePreservedPath(root, join(dirPath, quarantineLeaf)));
      }
      continue;
    }
    ...
    if (removal !== true) {
      ...
      if (removal?.quarantinePath) {
        preserved.push(relativePreservedPath(root, removal.quarantinePath));
      }
      continue;
    }
