...
// Mirrors fs-atomic.js's isOwnershipLoss: a lease/generation loss must keep
// propagating to the installer transaction, never be swallowed as a race.
...
// A concurrently vanishing entry is not an error, whatever errno the platform
// picks for it: captureRegularFileSnapshot() throws the managed-conflict
// error when the leaf disappears between its two lstats, and the removal
// primitives can surface a raw ENOENT/ENOTDIR for the same race. Both mean
// "the orphan is already gone or changed under us" and must preserve-and-
// continue like every other race in these loops.
function isVanishedLeafError(error) {
  if (error?.code === 'ENOENT' || error?.code === 'ENOTDIR') return true;
  return error?.message === 'managed destination leaf changed concurrently; refusing operation';
}

export {
  AtomicOwnershipLostError,
  writeFileAtomic,
  readFileMaybe,
  captureRegularFileSnapshot,
  regularFileIdentity,
...
export function pruneOrphans(dir, knownNames, sentinelSet, options = {}) {
  const assertOwnership = options.assertOwnership ?? options.recovery?.assertOwnership;
  assertOwnership?.();
  let pruned = 0;
  const preserved = [];
  const recovery = [];
  const maintenance = emptyMaintenanceReport();
  const maintenanceReport = maintainRecoveryArtifacts(dir, {
    ...(options.recovery ?? {}),
    assertOwnership,
  });
  ...
  try {
    ...
    // On Windows a concurrently vanished directory surfaces as a transient
    // EPERM on the read, not ENOENT; both mean "nothing to prune here".
    if (e.code === 'ENOENT' || e.code === 'EPERM') return { pruned, preserved, recovery, maintenance };
    throw e;
  }
  for (const entry of entries) {
    ...
    if (ownership === Ownership.mine || ownership === Ownership.legacy) {
      options.testInterlock?.('prune-before-remove');
      ...
      let result;
      try {
        result = removeOwnedRegularFile(path, snapshot.expectedDestination, {
          assertOwnership, testInterlock: options.testInterlock,
        });
      } catch (error) {
        if (isOwnershipLoss(error) || !isVanishedLeafError(error)) throw error;
        result = false;
      }
      if (result === true) {
        pruned++;
...
export function pruneOrphanDirs(root, knownNames, manifestLeaf, sentinelSet, options = {}) {
  ...
  for (const entry of entries) {
    ...
    const maintenanceReport = maintainRecoveryArtifacts(dirPath, {
      ...(options.recovery ?? {}),
      assertOwnership,
    });
    ...
    try {
      ...
      if (e.code === 'ENOENT' || e.code === 'EPERM') { addUnique(preserved, entry.name); continue; }
      throw e;
    }
    ...
    let removal;
    try {
      removal = removeOwnedRegularFile(manifestPath, manifestExpected,
        { assertOwnership, testInterlock: options.testInterlock });
    } catch (error) {
      if (isOwnershipLoss(error) || !isVanishedLeafError(error)) throw error;
      removal = false;
    ...
    options.testInterlock?.('prune-before-rmdir');
    if (removeEmptyDirectory(dirPath, { assertOwnership, testInterlock: options.testInterlock })) pruned++;
