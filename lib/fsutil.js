...
export function pruneOrphans(dir, knownNames, sentinelSet, options = {}) {
  ...
  const preserved = [];
  const recovery = [];
  const maintenance = maintainRecoveryArtifacts(dir, options.recovery ?? {});
  for (const path of maintenance.recovery) addUnique(recovery, path);
  for (const path of maintenance.preserved) addUnique(preserved, path);
...
export function pruneOrphanDirs(root, knownNames, manifestLeaf, sentinelSet) {
  ...
  for (const entry of entries) {
    ...
    const maintenance = maintainRecoveryArtifacts(dirPath);
    for (const path of maintenance.recovery) {
      const rel = relativePreservedPath(root, path);
      if (rel !== null) addUnique(recovery, rel);
    }
    for (const path of maintenance.preserved) {
      const rel = relativePreservedPath(root, path);
