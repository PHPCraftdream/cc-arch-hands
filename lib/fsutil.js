...
export function removeOwnedRegularFile(path, expected, options = {}) {
  ...
  try {
    if (!renameForRemoval(path, quarantine, expected, parentIdentities)) return false;
  } catch (e) {
...
function restoreWithoutOverwrite(from, to) {
  ...
  // already occupies the target, leave the source in quarantine: restoration
  // must never overwrite the successor or discard the displaced file.
  try {
    linkSync(from, to);
    try {
      unlinkSync(from);
    } catch {
      // The restored entry is safe even if quarantine cleanup races or fails.
    }
    return true;
  } catch {
    try {
      ...
      try {
        ...
        // Preserve either copy if quarantine cleanup races or fails.
      }
      return true;
    } catch {
