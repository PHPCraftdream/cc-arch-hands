...
import {
  ...
  sameFileIdentity, sameStatInteger, isOlderThan, lstatMaybe,
...
export function removeOwnedRegularFile(path, expected, options = {}) {
  ...
  let currentSnapshot;
  try { currentSnapshot = captureRegularFileSnapshot(path); }
  catch (error) {
    try { releaseReservation(); } catch { /* preserve the inspection error */ }
    throw error;
  }
  const expectedInfo = normalizeExpectedDestination(expected);
  const effectiveExpected = currentSnapshot.present
    ? { ...expectedInfo, contentDigest: expectedInfo.contentDigest ?? currentSnapshot.contentDigest,
      contentBytes: expectedInfo.contentBytes ?? currentSnapshot.contentBytes }
    : expectedInfo;
  if (!currentSnapshot.present || !sameFileIdentity(
    currentSnapshot.expectedDestination.identity, effectiveExpected.identity,
  ) || currentSnapshot.contentDigest !== effectiveExpected.contentDigest
      || currentSnapshot.contentBytes !== effectiveExpected.contentBytes) {
