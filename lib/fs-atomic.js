...
// Mirrors LEASE_QUARANTINE_DIR in lib/lease-lock.js. Hardcoded (not imported)
// so the mirrored companion-runtime import graph stays unchanged.
const LEASE_QUARANTINE_DIR = '.cah-lease-quarantine';
// The lease-quarantine namespace belongs here so isQuarantineName() and
// describeRecoveryArtifact() agree on what the string means.
const RECOVERY_MARKERS = [QUARANTINE_MARKER, PUBLICATION_FENCE_SUFFIX, '.cah-tmp-',
  LEASE_QUARANTINE_DIR];
...
function reclaimableUnprovedPublicationFence(artifact) {
  ...
  return Boolean(current?.isDirectory() && sameStatIdentity(current, artifact.identity)
    ...
    && isOlderThan(current, Date.now(), FENCE_STALE_MS));
}
