...
export function enumerateRecoveryArtifacts(root, options = {}) {
  // Displaced payloads, maintenance namespaces, and private temps have
  // different safety properties. Keep independent budgets so a directory
  // full of empty namespaces cannot hide the only surviving user data.
  const baseLimit = options.limit;
  const displacedLimit = boundedLimit(
    options.displacedLimit ?? options.payloadLimit ?? baseLimit, RECOVERY_LIMIT,
  );
  const namespaceLimit = boundedLimit(
    options.namespaceLimit ?? options.nonDisplacedLimit ?? options.payloadLimit ?? baseLimit,
    RECOVERY_LIMIT,
  );
  ...
  for (const entry of entries) {
    ...
    const bucket = artifact.kind === 'temp'
      ? temps : artifact.displacedData ? displaced : namespaces;
    const limit = artifact.kind === 'temp'
      ? tempLimit : artifact.displacedData ? displacedLimit : namespaceLimit;
    if (bucket.length < limit) bucket.push(artifact);
  }
  return [...displaced, ...namespaces, ...temps];
}
