...
import {
  chmodSync, closeSync, constants, lstatSync, mkdirSync, openSync, opendirSync, readFileSync,
...
export function enumerateRecoveryArtifacts(root, options = {}) {
  ...
  if (!directory) return recoveryArtifactList(state);
  ...
  scanRecoveryCategory(directory, 'displaced', state, (artifact) => {
    if (artifact.displacedData) addRecoveryArtifact(displaced, artifact, wanted);
  ...
  scanRecoveryCategory(directory, 'namespace', state, (artifact) => {
    if (!artifact.displacedData && artifact.kind !== 'temp') {
  ...
  scanRecoveryCategory(directory, 'temp', state, (artifact) => {
    if (artifact.kind === 'temp') addRecoveryArtifact(temps, artifact, wanted);
  }, boundedLimit(options.tempVisitLimit ?? options.visitLimit, RECOVERY_VISIT_LIMIT));
  return recoveryArtifactList(state, [...displaced, ...namespaces, ...temps]);
}

function recoveryArtifactList(metadata, values = []) {
  for (const [key, value] of Object.entries(metadata)) Object.defineProperty(values, key, {
...
function scanRecoveryCategory(directory, category, state, visit, requestedVisitLimit) {
  ...
  const maxVisits = isDisplaced && process.env.CAH_TEST_ONLY === '1'
    ? parseTestVisitLimit('displaced', RECOVERY_DISPLACED_VISIT_LIMIT)
    : isDisplaced ? RECOVERY_DISPLACED_VISIT_LIMIT : RECOVERY_VISIT_LIMIT;
  ...
  try {
    while (visits < categoryLimit) {
      let entry;
      try {
        if (recoveryFailureInjected('read')) throw injectedRecoveryError('read');
        entry = handle.readSync();
      } catch (error) {
        recordRecoveryFailure(state, directory, error);
        break;
      }
      if (entry === null) break;
      visits++;
    ...
    if (categoryLimit > 0 && visits >= categoryLimit) state.truncated = true;
  } finally {
...
function recoveryFailureInjected(kind) {
  if (process.env.CAH_TEST_ONLY !== '1') return false;
  const values = [
    process.env.CAH_TEST_ONLY_FSUTIL_RECOVERY_FAILURE,
    process.env.CAH_TEST_ONLY_FSUTIL_RECOVERY_EACCES === '1' ? 'all' : '',
...
function recoveryDirectory(root, state = null) {
  ...
  return observed === undefined || observed === null ? null
    : observed.isDirectory() ? root : dirname(root);
}
