...
function recoverOccupiedFence(destPath, fencePath, options) {
  ...
  if (publication) {
    ...
    assertOwnership(options);
    if (committed) {
      // deferCommitted is set only by the maintenance sweep, never by
      // beginFence(): an acquirer must finish a committed predecessor's
      // fence unconditionally — including one this same process created —
      // or a re-publication would spin against a fence nobody will clean.
      // Maintenance instead backs off while the publisher that already
      // renamed its payload is still live or fresh; reaping the fence here
      // turns that publisher's own cleanup into a spurious recovery error.
      if (options.deferCommitted
