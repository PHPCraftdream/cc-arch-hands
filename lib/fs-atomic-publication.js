...
function fenceIsStale(path, proof = null) {
  ...
  if (Number.isSafeInteger(proofTime)) {
    ...
    return Date.now() - proofTime >= FENCE_STALE_MS;
  }
...
function recoverOccupiedFence(destPath, fencePath, options) {
  ...
  if (publication) {
    ...
    if (committed) {
      ...
      // proofOwnerIsAlive() answers by pid alone, so a recycled pid keeps a
      // crashed publisher's committed fence looking live forever. Bound both
      // committed deferrals by staleness: a live peer gets its window-B
      // grace, a dead predecessor's fence must stay reclaimable. The old
      // trailing disjunct is redundant here — each deferral already implies
      // it — so it is not repeated.
      const deferCommittedLive = options.deferCommitted && !fenceIsStale(fencePath, proof);
