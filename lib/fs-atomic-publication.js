...
import { leaseExpired } from './lease-clock.js';
...
// A committed fence whose owner cannot be verified beyond pid liveness (a
// lease-less proof) is deferred for this long before maintenance or an
// acquirer may reclaim it. It must exceed any realistic single maintenance
// pass over a live publisher, and a reclaim remains safe for a genuinely
// live owner because finishPublication() tolerates a benign third-party
...
function lstatMaybe(path) {
  try { return lstatSync(path, { bigint: true }); } catch (error) {
    // A concurrent unlink of the exact path can surface as a transient EPERM
    // on Windows (delete-pending) rather than ENOENT, matching the EPERM
    // tolerance already applied to reads (readProofBytes) and directory
    // emptiness checks (directoryIsEmpty) for the same phenomenon. Every
    // caller here treats null as "not stably present right now" in a racy
    // transaction, which a delete-pending path already is.
...
// renewLease() applies. After a crash the owner file sits there untouched
// with the matching token and generation, so continuity alone would defer
// the fence forever; a recycled pid never reacquired that token, and the
// next legitimate holder of the lease overwrites the owner file with its
// own identity, breaking the match and making the fence reclaimable again.
...
// error to surface.  On Windows a concurrent unlink of the proof file can
// surface as a transient EPERM on open (delete-pending), which is why EPERM
// is tolerated here alongside ENOENT.  Directory reads in the same recovery
// path (readdirSync/opendirSync) can surface EPERM the same way, but those
// guards admit only ENOENT today and rely on their callers' conservative
// preserve-and-report behavior; do not read this comment as a claim about
// them.
...
function recoverOccupiedFence(destPath, fencePath, options) {
  ...
  if (publication) {
    ...
    if (committed) {
      ...
      // (deferForeignCommitted) must finish a fence this same process
      // created unconditionally — proofOwnerIsAlive() is always true for
      // our own pid, so a same-process deferral could never expire — but
      // it defers a foreign, live owner: reaping a live peer's committed
      ...
      // alone. Two signals decide: lease continuity defers the fence only
      // while the recorded owner file still carries the proof's exact token
      // and generation AND its heartbeat is fresh — committedLeaseStillHeld()
      // applies the same expiry rule renewLease() does, so an abandoned lease
      // nobody renews stops deferring once the heartbeat passes the lease
      // period — and a lease-less proof keeps a freshness window widened to
      // COMMITTED_DEFER_MS. A dead publisher's fence stays reclaimable either
      // way: its lease is superseded or expires, its pid dies, and an aged
      // lease-less proof falls out of the window. The old
      // trailing disjunct is redundant here — each deferral already implies
