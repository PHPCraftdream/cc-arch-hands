...
// verifiably the same live transaction, however slow: the lease owner file
// still carries the proof's own token and generation. A recycled pid never
// reacquired that token, and the next legitimate holder of the lease
// overwrites the owner file with its own identity, breaking the match and
// making the fence reclaimable again.
...
function recoverOccupiedFence(destPath, fencePath, options) {
  ...
  if (publication) {
    ...
    if (committed) {
      ...
      // alone. Two signals decide: lease continuity defers the fence with no
      // age cutoff for as long as the recorded owner file still carries the
      // proof's exact token and generation, and a lease-less proof keeps a
      // freshness window widened to COMMITTED_DEFER_MS. A dead publisher's
      // fence stays reclaimable either way: its lease is superseded or its pid
      // dies, and an aged lease-less proof falls out of the window. The old
