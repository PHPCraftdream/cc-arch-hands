...
const FENCE_TRANSIENT_WINDOW_MS = 2_000;
...
function lstatMaybe(path) {
  try { return lstatSync(path, { bigint: true }); } catch (error) {
    ...
    // tolerance applied to reads (readProofBytes) and directory emptiness
    // checks (directoryIsEmpty). Every caller treats null as "not stably
    // present right now" in a racy transaction, which delete-pending already is.
