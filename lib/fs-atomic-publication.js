...
function verifiedLifecycleSuccessor(proof, options) {
  const lease = lifecycleLeaseFor(options);
  if (!lease || proof?.leasePath !== lease.path) return false;
  // Do not trust metadata supplied by a caller until the live owner file
  // proves that this exact token and generation still own the lease path.
  const current = readLeaseOwnerForRecovery(lease.path);
  if (!current || current.token !== lease.token || current.generation !== lease.generation) {
    return false;
  }
  return proof.leaseToken !== lease.token || proof.leaseGeneration !== lease.generation;
}
...
function validProofShape(proof) {
  ...
  return leaseFields.length === 0 || (leaseFields.length === 3
...
function publicationProofState(fencePath) {
  ...
  // A complete proof plus a corrupt/partial sibling is not a valid two-proof
  // transaction. Keep the fence occupied so an unproved sibling cannot be
  // mistaken for disposable installer state.
  if (candidates.some((candidate) => candidate.present
      && (!candidate.identity || !validProofShape(candidate.proof)))) return null;
  const validCandidates = candidates.filter((candidate) => candidate.identity);
  if (!validCandidates.length) return null;
  const first = validCandidates[0].proof;
  // Both names may exist during an interrupted replacement.  They are one
  // recoverable state only when they describe the same transaction; an
  // unrelated or partial second file remains evidence of incomplete recovery.
  if (validCandidates.length > 1
      && validCandidates.some((candidate) => proofCore(candidate.proof) !== proofCore(first))) return null;
  ...
  return { proof: validCandidates[0].proof, candidates: validCandidates };
}
...
// newest complete state only after validating both copies as one transaction.
...
function publicationProofFor(destPath, tempPath, tempIdentity, expected, fencePath, options) {
  const lifecycleLease = lifecycleLeaseFor(options);
  return {
