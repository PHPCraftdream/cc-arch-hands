...
// error to surface.  On Windows the same race surfaces as a transient EPERM
// on open (already in RETRY_CODES), not ENOENT.
...
function recoverOccupiedFence(destPath, fencePath, options) {
  ...
  if (publication) {
    ...
    if (committed) {
      ...
      // crashed publisher's committed fence looking live forever. Bound the
      // foreign deferral by staleness: a live peer gets its window-B grace,
      // a dead predecessor's fence must stay reclaimable.
      const deferForeignLive = options.deferForeignCommitted && foreignLiveOwner
        && !fenceIsStale(fencePath, proof);
      if ((options.deferCommitted || deferForeignLive)
