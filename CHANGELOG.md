...
- Added optional Codex agents for `gpt-6-sol` and `gpt-6-luna` at `low`,
  `medium`, `high`, `xhigh`, and `max` effort.

### Changed

- Sol and Luna's unversioned aliases (`ls`/`ll`, etc.) now target GPT-6.
  Their previous GPT-5.6 agents remain available with `1` suffixes
  (`ls1`/`ll1`, etc., including `us1`/`ul1` for `ultra`).
